...
\file{.bib} files) and as many as you like can be used:
...
File patterns can be used too\footnote{These are full BSD-style file glob
  patterns.}:

\begin{ltxexample}[style=latex]{}
  \addbibresource{file*.bib}
  \addbibresource{file?.bib}
  \addbibresource{file{1,2,3}.bib}
\end{ltxexample}
  
\subsubsection{Use remote data sources?\msecpref{use:bib:res}}

Remote datasources can be fetched via HTTP(S) or FTP(S):
...
It is possible to change the data in almost arbitrary ways as it is read,
without modifying the data files. This is useful, for example for data
...
macro. Here is a simple example--suppose that the data source file
\file{file1.bib} had \bibtype{article} entries with large or badly
formatted \bibfield{abstract} fields, you could remove these as the data is
read like this:
...
\subsubsection{Sort the bibliography correctly for a particular language?\msecpref{bib:cav:enc}}

Sorting can be tailored to specific languages\footnote{Actually, <locales>
  but \biber maps babel/polyglossia language names to locale names
  automatically} easily when using \sty{babel} or \sty{polyglossia} with
the option:
...
\end{ltxexample}
%
The sorting locale can also be specified explicitly and with more
granularity than this global setting example.\msecpref{aut:ctm:srt}
