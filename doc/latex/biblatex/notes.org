...
** TODO Seems to be somewhere where \endgroup is being missed with language=auto*
** TODO langidopts seems to not be reset, next entry with no langidopts when using autolang=langname re-uses last field which might not be right (variant= in german, for example)
* Documentation
