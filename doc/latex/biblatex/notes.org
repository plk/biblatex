...
** Need \multiscriptlang which is initialises from blx@languagename and overriden by per-entry multiscriptlang
** Same for \multscriptform - should be overrided per-entry
** Should we default to blx@languagename? This means you can't \printx[][form=translated]{} and pick up an arbitrary translated form
* Documentation
...
** \loopforms, \looplangs, \loopformlangs and \loopvariants (loop over all forms/langs doing macro for each)
** \getlang
** \formcount, \langcount, \formlangcount (all in formats, latter uses selected form)
