...
* They determine data and so multiple settings in refcontexts would need to create multiple data
  versions in the .bbl. The only multiple data context for an entry in the .bbl is a sorting list.
