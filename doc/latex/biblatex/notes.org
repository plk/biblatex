...
Do these really need form/lang per field? They are all processed by biber so if they
need it, it needs passing in .bcf section. How could refcontext determine these?
...
They probably do need these so that they can be chosen globally since they shouldn't change across
refcontexts otherwise it would be very confusing since different cites/bibiographies would have
different values. sorting specs are ok to have form/lang since without this you can't fine tune
sorting specs.

Sorting specs form/lang have overrides thus:

global
  refcontext
    per-field

because a printbibliography/printbiblist happens inside a refcontext.

The others have no macro which occurs in a refcontext and so they have:

global
  per-field


What does DeclareVariantFallbacks do?

8148 - add form and lang as refcontext options
       replace all blx@vform/blx@vlang with \blx@refcontext@form/\blx@refcontext@lang

labeltitle/labelname without form/lang but use label*source{field} in bltx to resolve these
to a form/lang so that they can be per refcontext


Current status: on dev, labelame and labeltitle are not generated by biber. Biber just finds
out the source field and biblatex resolves this dynamically per get_data for an entry. This allows
it to resolve these in a refcontext. This means nothing in dev since a refcontext is only a sorting
spec which is irrelevant to determining labeltitle and labelname.

In ms, there is a problem. labeltitle and labelname are used to determing many things like hashes,
extra* tracking etc. These are static information for biblatex as they appear in the .bbl. If
labeltitle and labelname can change with refcontext, this means that biber should generate lists
for every refcontext combination, not just for each sorting spec.

Currently \printbibliography determines which sortlists biber creates but it should be refcontexts
since these may contain no printbibliography.
...
the cs should also include the refcontext form/lang

but then sortlists should be determined by refcontext macros and not printbibliography

This:

  \csxappto{blx@xml@slistsbib}{%
    \blx@xml@slist{\blx@refcontext@sorting}{entry}{\blx@tempe}{\blx@refcontext@sorting}{\blx@tempd}{}{\blx@vform}{\blx@vlang}}%

should be moved to refcontext and the vform/vlang takedn from refcontext settings (or global). No
more \vform or \vlang macros.

What about the calls to retrieve labelname which determine the hashes, extras etc. which form/lang should
they use? Presumably something from every refcontext so that all possible cominations are in the .bbl.
