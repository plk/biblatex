...
** TODO Need \multiscriptlang which is initialised from blx@languagename and overriden by per-entry multiscriptlang - currently stomping on global value
** TODO Same for \multiscriptform - should be overrided per-entry
** TODO Should we default to blx@languagename? This means you can't \printx[][form=translated]{} and pick up an arbitrary translated form
* Documentation
** TODO Babel adjusts to lang of fields (hyphens, shorthands, extras, just hyphens if lang isn't loaded)
** TODO Biber makes sure that lang isn't present as option for fields where the lang matches the hyphenation field
