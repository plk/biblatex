...
* Documentation
** TODO Did I mention why it doesn't make sense to ms related functionality?
** TODO \DeclareLabel* are ms compatible \field[]{} doc needs enhancing.
** TODO Sorting/Inheritance macros are ms enabled
** TODO In RELEASE - incompatible change - language=auto -> language=autobib for styles which set this
** TODO msform and mslang are global and per-entry. mslang defaults to babel/polyglossia setting or global language option. msform to "original"
** TODO mstranslang is global and per-entry., defaults in same way as mslang
** TODO LANGID field also sets per-entry default multiscript lang
** TODO global multiscriptform option - multiscriptlang comes from language/autolang option
...
** TODO Biber makes sure that lang isn't present as option for bibfields where the lang matches the langid field
...
** TODO DM needs documenting so people can tell what ms fields are (just add marker to fields in doc)
...
** TODO \printfield obeys lang opts, printlist, name don't (no point)
