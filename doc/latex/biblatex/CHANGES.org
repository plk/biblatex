...
- Three new delimiters are introduced. ~authortypedelim~, ~editortypedelim~,
  ...
  This together with ~\DeclareFieldFormat{<name>type}~ allows to go from
  'E. Ditor, ed.' to 'E. Ditor (ed.)' more easily without the need
  to redefine entire bibmacros.
- \DeclareDelimAlias allows to alias one delimiter to another.
...
- ~\mkpagetotal~ now has its own bibstrings ~<pagination>total~
  and ~<pagination>totals~. Previously it relied on the
  ~<pagniation>(s)~ bibstrings.
