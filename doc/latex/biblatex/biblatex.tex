...
\optitem[default]{namestrunchandling}{\opt{default}, \opt{nohash}, \opt{nosort}, \opt{nohashnosort}}

By default, name lists which are truncated with <et al>--either explicitly by <and others> in the data source or the \opt{uniquelist} and \opt{min/maxnames} options--result in different name list hashes (and therefore different \opt{extraname} and \opt{extradate} values) and different sorting. This option allows this behaviour to be tuned. \opt{nohash} makes \biber ignore <et al> truncations for the purposes of generating name list hashes. Consider:
...
\end{lstlisting}
%
With \kvopt{maxnames}{3}, \kvopt{minnames}{1}, \kvopt{namestrunchandling}{default}, the result would be:
...
\end{lstlisting}
%
Whereas with \kvopt{maxnames}{3}, \kvopt{minnames}{1}, \kvopt{namestrunchandling}{nohash}, the result would be:
...
\end{lstlisting}
%
The option \opt{nosort} has a related effect on sorting--the visible list of names (which is the \opt{minsortnames} value) used to determine sorting will ignore any truncation. This means that with \kvopt{namestrunchandling}{nosort}, the name lists:
...
\end{lstlisting}
%
will sort exactly the same. The default setting of \opt{namestrunchandling} always sorts in the order shown in the example, that is, by default, truncated names lists always sort after any name lists identical to the point of truncation.

\kvopt{namestrunchandling}{nohashnosort} combines the effect of \opt{nohash} and \opt{nosort}.

Note that the \opt{namestrunchandling} option will affect the \bibfield{extradate} and \bibfield{extraname} fields.
...
minitems	&\+&\+&\_&\+&\+&\_&\_\\
minnames	&\+&\+&\_&\+&\+&\_&\_\\
namestrunchandling	&\+&\_&\_&\_&\_&\_&\_\\
nametemplates &\_&\_&\+&\_&\+&\+&\+\\
natbib		&\+&\_&\_&\_&\_&\_&\_\\
...
\item Added \opt{namestrunchandling}\see{use:opt:pre:int}
