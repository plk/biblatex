...
\end{valuelist}
%
Note that \opt{edtf} format will enforce \kvopt{dateera}{astronomical}, \kvopt{datezeros}{true}, \kvopt{timezeros}{true}, \kvopt{seconds}{true} and \kvopt{$<$datetype$>$time}{24h}. \opt{ymd} is an ETFT-like format but which can change the various options which the strict \opt{edtf} option does not allow for.
