...
\item[biblatexml]\BiberOnlyMark  Experimental XML format for \biblatex.
...
\subsection{Handling Names}\BiberOnlyMark
\label{use:names}
Historically, name handling in \latex bibliographies has been determined by the name parsing rules of \bibtex. In a modern context, this name handling is rather archaic, restricted to Western names with the four parts <family>, <given>, <prefix>, <suffix> (often referred to in \bibtex documentation as <last>, <first>, <von> and <Jr> parts). When using \biber, it is possible to define arbitrary name parts since the allowable name parts are defined in the data model. The default definition is:
...
\end{lstlisting}
%
Which corresponds to the standard \bibtex supported name parts. There is no possibility, when using 
\bibtex format \file{.bib} data sources, of expanding the range of supported name part because these parts are hard-coded into the \bibtex program/library name parsing routines. When using other data sources such as \biblatexml, users can define and use arbitrary name parts. Here is an example of defining and using a <patronymic> name part in Russian.

Firstly, enhance the default data model with the new namepart by putting the following in a datamodel definition file (see \secref{aut:ctm:dm}):

\begin{lstlisting}[style=latex]{}
\DeclareDatamodelConstant[type=list]{nameparts}
                         {prefix,family,suffix,given,patronymic}
\end{lstlisting}
%
This automatically creates the relevant code in \biblatex for handling the new name part. Consider the following \biblatexml data source:
{\small
...
%
Notice that we have defined <patronymic> name part types in the datasource which is now allowable due to the enhanced datamodel constant definition. Then, define a name format to use this new name part:
...
\DeclareNameFormat{author}{%
  \setkeys{blx@opt@namepart}{#1}%
   ...
   \mkbibnamefamily{\blx@namepart@family}%
   \ifempty{\blx@namepart@given}
     {}
     {\revsdnamepunct
      \bibnamedelimd
      \mkbibnamegiven{\blx@namepart@given}%
      \space
      \mkbibnamepatronymic{\blx@namepart@patronymic}\isdot}
   \usebibmacro{name:andothers}}
\makeatother
\end{lstlisting}
%
Notice that the macro \cmd{mkbibnamepatronymic} (as with all \cmd{makebibname*} macros) was automatically created by the datamodel <nameparts> constant definition. We would probably like to make the \biber sorting system aware of this new name part and so we could define, for example (see \secref{aut:ctm:srt}):
...
\end{lstlisting}
%
The result of a simple document with this datasource would be:

\begin{mdframed} 
Толстой (1877) and Булгáков (1966)\\[1ex]
\textbf{References}\\[1ex]
Булгáков, Михаил Афанáсьевич (1966). Мастер и Маргарита.\\
Толстой, Лев Николаевич (1877). Война и мир.
\end{mdframed}
...
\begin{lstlisting}[style=latex]{}
\blx@namepart@<namepart>
\blx@namepart@<namepart>i
\end{lstlisting}
%
...
\DeclareNameFormat{author}{%
  ...
  \ifthenelse{\value{listcount}=1}
    {\blx@namepart@family%
     \ifblank{\blx@namepart@given}{}{\addcomma\space\blx@namepart@given}}
    {\ifblank{\blx@namepart@given}{}{\blx@namepart@given\space}%
     \blx@namepart@family}%
  \ifthenelse{\value{listcount}<\value{liststop}}
    {\addcomma\space}
    {}}
...
Defines the name list format \prm{format}. This formatting directive is arbitrary \prm{code} to be executed for every name in a list processed by \cmd{printnames}. If an \prm{entrytype} is specified, the format is specific to that type. The \prm{entrytype} argument may be a comma"=separated list of values. The individual parts of a name will be passed to the \prm{code} as a comma"=separated list which should be processed by \cmd{setkeys} (see example above). The default data mode defines four name part which correspond to the standard \bibtex name parts arguments:
