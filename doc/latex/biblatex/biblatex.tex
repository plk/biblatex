...
\item The \kvopt{form}{translated} fields are all implicitly \kvopt{lang}{english} due to the global \opt{vtranslang} setting. Note that \biber has therefore omitted the explicit language setting for the \bibfield{title} field in the data source since it matches \opt{vtranslang}.
\end{itemize}
...
\end{lstlisting}
%
Here the |*| marks a null list element. The particular character which marks this in a name list should correspond to the \biber option |--variant-null-list| (the default is |*|). Such name elements will be blank in the \file{.bbl} (\bibtex parsing rules mean they can't be blank in the \file{.bib} itself, hence the marker) and so you can test for them in the element format and omit them entirely:
...
\end{lstlisting}
%
This would result in:\\[2ex]
Булгаков, Павел (Bulgakov, Pavel) and Розенфельд, Борис (Rosenfeld, Boris) and John Smith
%
When sorting using lists with null elements, \biber uses instead the corresponding element from the default variant for the list being used. For example when sorting using the |uniform| form of the \bibfield{author} field:
...
\end{lstlisting}
%
\biber will sort this |uniform| field as if it were:

\begin{lstlisting}[style=bibtex,escapechar=+]
  AUTHOR_uniform = {Bulgakov, Pavel and Rosenfeld, Boris and John Smith},
\end{lstlisting}
