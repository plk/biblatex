...
\end{lstlisting}
%
Here the |*| marks a null list element. The particular character which marks this in a name list should correspond to the \biber option |--variant-null-list| (the default is |*|). \biber will resolve such null list elements using the fallback chain specified with the global \cmd{vform} and \cmd{vlang} settings.
...
\valitem{form}{\prm{form}}
\valitem{lang}{\prm{lang}}

For constraints of \prm{type} <data> and \prm{datatype} <pattern>, constrain only specific variant forms or languages of the field. For allowed variant forms, see \secref{bib:fld:var}. Specifying neither of these for a variant enabled field will perform the pattern validation on all variants. Specifying only one of them will perform the validation on all forms (for a given language) or 
languages (for a given form).
