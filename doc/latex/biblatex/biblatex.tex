...
Setting \opt{vlang} to <\texttt{*}> means that \biblatex will use whichever language is found for a field variant with form \opt{vform}. This assumes of course that your entry contains only one language variant with form \opt{vform} otherwise the behaviour will not be well defined. For example:
...
\end{lstlisting}
%
With fallback enabled and global variant settings \opt{vform} <original> and \opt{vlang} <\texttt{*}>, the selected \bibfield{BOOKTITLE} and \bibfield{TITILE} fields for printing will be:
...
\end{lstlisting}
%
This is because the <original> language variants will be selected due to the \opt{vlang} wildcard. If we reset the current \opt{vform} to <uniform> in the document with \cmd{vform}, then we would get:
\begin{lstlisting}[style=latex,escapechar=+]
TITLE     = Acta Barnabae
BOOKTITLE = Acta Apocryphorum Apostolorum
\end{lstlisting}
%
This is because the <uniform> variant of \bibfield{TITLE} would be selected, regardless of the language, due to the \opt{vlang} wildcard but the \bibfield{BOOKTITLE} would fall back to the global <original> form and then the \opt{vlang} wildcard would select whatever language variant of the <original> form exists.
