...
\item If \texttt{match} is defined but
  ...
  \texttt{match} regular expression (logic is reversed if you use \texttt{notmatch} instead)\footnote{Regular expressions are full Perl 5.14 regular expressions. This means you may need to deal with special characters, see examples.}. You may use capture parenthesis as usual and refer to these (\$1\ldots\$9) in later \texttt{fieldvalue} specifications. This allows you to pull out parts of some fields and put these parts in other fields.
...
\boolitem[false]{multiscript}

The field can occur multiple times in the data source with different script variants and/or languages.
...
\DeclareDatamodelFields[type=field, datatype=literal, multiscript]{<<field4>>}
...
\item One of the valid literal fields (\bibfield{field4}) is a multiscript field which can occur several times in a datasource with different script/language variants
...
Executes \prm{true} if the current list is \prm{name list}, and \prm{false} otherwise. This command is robust. It is intended for use in name formatting directives and always executes \prm{false} when used in any other context.
...
\item Retired \cmd{DeclareFieldInputHandler}, \cmd{DeclareListInputHandler} and \cmd{DeclareNameInputHandler}.
\end{release}
