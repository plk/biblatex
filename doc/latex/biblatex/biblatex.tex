...
This command prints either the \bibfield{labeltitle} field (\bibtex) or the title field selected with \cmd{DeclareLabeltitle} (\biber, see \secref{aut:ctm:fld}). It will use the abridged title in the \bibfield{shorttitle} field, if available. The starred variant always prints the full title found in the \bibfield{title} field.
...
When selecting a particular language variant of the field using the \prm{form} and/or \prm{lang} options, the \opt{autolangshort} package option determines which Babel/Polyglossia language environment to use when printing the field. The \opt{autolangshortstrings} package option determines whether localization string switching will be performed in addition to Babel/Polyglossia language environment switching. See the example in \cmd{printlist} above.
