...
It is also possible to omit a particular name element variant in case it is not needed:
...
\end{lstlisting}
%
Here the |*| marks a null name element. The particular character which marks this in a name list should correspond to the \biber option |--variant-null-name| (the default is |*|). Such name elements will be blank in the \file{.bbl} (\bibtex parsing rules mean they can't be blank in the \file{.bib} itself, hence the marker) and so you can test for them in the element format and omit them entirely:
...
Булгаков, Павел (Bulgakov, Pavel) and Розенфельд, Борис (Rosenfeld, Boris) and John Smith\\[2ex]
