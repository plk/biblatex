...
\optitem{subtitle, titleaddon, language, version, note, organization, date, month, year, addendum, pubstate, doi, eprint, eprintclass, eprinttype, urldate}
...
Executes \prm{true} if the date field \prm{datetype} has an annotation \prm{annotation} and false otherwise. The \prm{datetype} argument is mandatory, because it cannot be inferred in most contexts where \cmd{ifdateannotation} will be used.
...
\cmditem{getfieldannotation}[field]

Retrieves any literal annotation for the field \prm{field}. If \prm{field} is not given, the current data field as indicated by \cmd{currentfield}, \cmd{currentlist} or \cmd{currentname} (see \secref{aut:bib:fmt}) is assumed. Of course, this is only possible if these commands are defined, that is, inside formatting directives.

\cmditem{getitemannotation}[field][item]

Retrieves any literal annotation for the item \prm{item} in the field \prm{field}. The optional argument \prm{field} can be inferred if not provided as with \cmd{getfieldannotation}. If \prm{item} is not given, the number of the item currently being processed as given by \cnt{listcount} is used.

\cmditem{getpartannotation}[field][item]{part}

Retrieves any literal annotation for the part \prm{part}. The two optional arguments \prm{field} and \prm{item} can be inferred as in \cmd{getitemannotation}. The parameter \prm{part} can never be inferred and is therefore a mandatory argument.
...
\cmditem{getdateannotation}{datetype}

Retrieve any literal annotation for the datefield \prm{datetype}. The \prm{datetype} argument is mandatory, because it cannot be inferred in most contexts where \cmd{getdateannotation} will be used.
...
\begin{changelog}
\begin{release}{3.12}{2018-??-??}
\item Added literal annotation functionality\see{use:annote}
