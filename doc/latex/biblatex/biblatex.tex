...
\item[\biber]
\biber is the default backend of \biblatex. You only need one backend, either \bibtex or \biber. \biber comes with TeXLive and is also available from SourceForge.\fnurl{http://biblatex-biber.sourceforge.net/}
...
\end{ltxsyntax}
%
Use \cmd{vfallbackon} to turn variant fallback on and \cmd{vfallbackoff} to turn it off. The default is \cmd{vfallbackon}. Variant fallback means that if a the current global variant is not found for a field, a fallback will be looked for using the global default form and lang (specified via the package options). Such a fallback ignores any currently modified global values set by using \cmd{vform}, \cmd{vlang} or \cmd{vtranslang}. Not that variant fallback does not apply when you explicitly specify a variant using the \opt{form/lang} arguments to a macro which allows variant specification; it is only used in cases where you specify no variant explicitly and rely on the current global settings. For example, perhaps you have an entry like this:
...
This option applies to numerical citation\slash bibliography styles only and requires that the \opt{defernumbers} option from \secref{use:opt:pre:gen} be enabled globally. If enabled, it will reset the numerical labels assigned to the entries in the respective bibliography, \ie the numbering will restart at~1. Use this option with care as \biblatex can not guarantee unique labels globally if they are reset manually.
...
\end{lstlisting}
%
A definition provided in the document preamble can be subsequently adopted with \texttt{\textbackslash ExecuteBibliographyOptions\{autocite=<<name>>\}} (see \secref{use:cfg:opt}).
...
Specifies the form for the variant enabled field specified with \opt{field}, if the datasource you are using supports this. If absent, default to the value of the \opt{vform} option. See \secref{bib:fld:var} and \secref{use:opt:pre:gen}. For allowed variant forms, see \secref{bib:fld:var}.

\choitem[none]{lang}{\prm{language}}

Specifies the \sty{babel}/\sty{polyglossia} language for the variant enabled field specified with \opt{field} if the datasource you are using supports this. If absent, default to the value of the \opt{vlang} option or \opt{vtranslang} if the \kvopt{form}{translated}. See \secref{bib:fld:var} and \secref{use:opt:pre:gen}.
...
\item Removed Zotero RDF XML and Endnotexml driver support for \biber.
\end{release}

\begin{release}{2.9}{2014-02-}
...
\item Added \bibfield{sortinithash} with\see{aut:bbx:fld:gen}\BiberOnlyMark
