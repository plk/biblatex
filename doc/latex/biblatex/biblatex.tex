...
\item[relatedstring] If an entry contains this field, then the localization key specified by its value is printed before data from the related entries. If the field does not specify a key, its value is printed literally. If both \bibfield{relatedtype} and \bibfield{relatedstring} are present in an entry, \bibfield{relatedstring} is used for the pre-data string (but \bibfield{relatedtype} is still used to determine the macro and format to use when printing the data).
\end{keymarglist}
