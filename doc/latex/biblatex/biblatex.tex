...
\item[Integer fields] hold unformatted integers which may be converted to ordinals or strings as they are printed. A typical example is the \bibfield{extrayear} field. Such fields are sorted as integers.

\item[Datepart fields] hold unformatted integers which may be converted to ordinals or strings as they are printed. A typical example is the \bibfield{month} field. For every field X of datatype \bibfield{date} in the datamodel, datepart fields are automatically created with the following names: \bibfield{Xyear}, \bibfield{Xendyear}, \bibfield{Xmonth}, \bibfield{Xendmonth}, \bibfield{Xday}, \bibfield{Xendday}, \bibfield{Xhour}, \bibfield{Xendhour}, \bibfield{Xminute}, \bibfield{Xendminute}, \bibfield{Xsecond}, \bibfield{Xendsecond}, \bibfield{Xtimezone}, \bibfield{Xendtimezone}.
...
Date fields such as the default data model dates \bibfield{date}, \bibfield{origdate}, \bibfield{eventdate}, and \bibfield{urldate} adhere to  \acro{EDTF}\footnote{\url{https://www.loc.gov/standards/datetime/pre-submission.html}} (Extended Date/Time Format) specification levels 0 and 1\footnote{With the exception of section 5.2.2}. \acro{EDTF} is a stricter subset of the somewhat messy formats allowed by \acro{ISO8601v2004} and is better suited to bibliographic data. In addition to the \acro{EDTF} empty date range markers, you may also specify an open ended date range by giving the range separator and omitting the end date (\eg \texttt{YYYY/}). See \tabref{bib:use:tab1} for some examples of valid date specifications and the formatted dates automatically generated by \biblatex. The formatted date is language specific and will be adapted automatically. If there is no \bibfield{date} field in an entry, \biblatex will also consider the fields \bibfield{year} and \bibfield{month} for backwards compatibility with traditional \bibtex but this is not encouraged as explicit \bibfield{year} and \bibfield{month} are not parsed for date meta-information markers and are used as-is. Style authors should note that date fields like \bibfield{date} or \bibfield{origdate} are only available in the \file{bib} file. All dates are parsed and dissected into their components as the \file{bib} file is processed. The date and time components are made available to styles by way of the special fields discussed in \secref{aut:bbx:fld:dat}. See this section and \tabref{aut:bbx:fld:tab1} on page~\pageref{aut:bbx:fld:tab1} for further information.

Date field names must end with the string <date>, as with the default date fields. Bear this in mind when adding new date fields to the datamodel (see \secref{aut:ctm:dm}).

\biblatex also supports dates before common era (BCE/BC) in the standard \acro{EDTF} negative date format. It also supports the \acro{EDTF} <approximate> (circa) and uncertain dates. Such date formats set internal markers which can be tested for so that appropriate localised markers (such as \opt{circa} or \opt{beforecommonera}) can be inserted. The table shows formats which use appropriate tests and formatting. See the date meta-information tests in \secref{aut:aux:tst} and the localisation strings in in \secref{aut:lng:key:era} and \secref{aut:lng:key:circa}. See also the \file{96-dates.tex} example file for a complete example of the tests and localisation strings use.

The output of <circa>, uncertainty and era information in standard styles (or custom styles not customising the internal \cmd{mkbibrange*} macros) is controlled by the package options \opt{datecirca}, \opt{dateuncertain}, \opt{dateera} and \opt{dateeraauto} (see \secref{use:opt:pre:gen}). See \tabref{bib:use:tab2} for examples which assumes these options are all used.
...
\cmidrule{1-1}\cmidrule(l){2-2}\cmidrule(l){3-3}
-0876			  & 877 BCE			     & using \cmd{ifdateera} test and \opt{beforecommonera} localisation string\\
...
1723\textasciitilde & c. 1723 & using \cmd{ifdatecirca} test\\
1723? & 1723? & using \cmd{ifdateuncertain} test\\
1723?~ & circa. 1723? & using \cmd{ifdateuncertain} and \cmd{ifdateirca} tests\\
...
\optitem[comp]{date}{\opt{year}, \opt{short}, \opt{long}, \opt{terse}, \opt{comp}, \opt{edtf}}
...
\item[edtf] Use Extended Date/Time Format\footnote{\url{https://www.loc.gov/standards/datetime/pre-submission.html}} (\texttt{yyyy-mm-dd}), for example:\par
2010-01-01\par
...
\optitem[year]{datelabel}{\opt{year}, \opt{short}, \opt{long}, \opt{terse}, \opt{comp}, \opt{edtf}}

Similar to the \opt{date} option but controls the format of the date field selected with \cmd{DeclareLabeldate}.

\optitem[comp]{origdate}{\opt{year}, \opt{short}, \opt{long}, \opt{terse}, \opt{comp}, \opt{edtf}}

Similar to the \opt{date} option but controls the format of the \bibfield{origdate}.

\optitem[comp]{eventdate}{\opt{year}, \opt{short}, \opt{long}, \opt{terse}, \opt{comp}, \opt{edtf}}

Similar to the \opt{date} option but controls the format of the \bibfield{eventdate}.

\optitem[short]{urldate}{\opt{year}, \opt{short}, \opt{long}, \opt{terse}, \opt{comp}, \opt{edtf}}
...
This option controls whether \texttt{short} and \texttt{terse} date components are printed with leading zeros.

\boolitem[true]{timezeros}

This option controls whether time components are printed with leading zeros.
...
This option controls whether to output <circa> information about dates. If set to \opt{true}, dates will be preceded by the expansion of the \cmd{datecircaprint} (\secref{use:fmt:fmt}).
...
\optitem{dateera}{\opt{secular}, \opt{christian}}

This option controls which localisation strings are used to format date era information (see \secref{aut:lng:key:era}). By default, this option is not set and no era information is output for dates. If set, the \cmd{dateeraprint} will be used to format the era information, see \secref{use:fmt:fmt}.

\intitem{dateeraauto}

This option sets the integer number for CE/AD years, below which CE/AD era localisation strings will automatically be added. This option does nothing without \opt{dateera} being set.
...
\end{itemize}
%
For example, if this option is enabled, \biblatex always precedes the last name with the prefix---Ludwig van Beethoven would be cited as «Beethoven» and alphabetized as «Beethoven, Ludwig van» by default. If this option is enabled, he is cited as «van Beethoven» and alphabetized as «Van Beethoven, Ludwig» instead.
This option is also settable on a per-type scope. With \biblatexml datasources and the extended name format supported by \biber, this is also settable on per-namelist and per-name scopes.
...
Command which formats the printing of date uncertainty information when the global option \opt{dateuncertain} is enabled. By default, prints the language specific \cmd{bibdateuncertain} string (\secref{use:fmt:lng}).

\csitem{datecircaprint}
Command which formats the printing of date uncertainty information when the global option \opt{datecirca} is enabled. By default, prints the <circa> localised term (\secref{aut:lng:key:circa}) and the \opt{datecircadelim} delimiter.

\csitem{dateeraprint}
Command which formats the printing of date era information when the global option \opt{dateera} is set. By default, prints the \opt{dateeradelim} delimiter and the appropriate localised era term (\secref{aut:lng:key:era}). Note that all dates are assumed to be AD/CE and this era is never output. In the event that you always want to include AD/CE eras in dates, customise this command.
...
This field holds the season component of the \bibfield{date} field as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). If the date is a range, it holds the season component of the start date.
...
If the date specification in the \bibfield{date} field is a range, this field holds the season component of the end date as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). A blank (but defined) \bibfield{endseason} component indicates an open ended \bibfield{date} range.
...
This field holds the season component of the \bibfield{origdate} field as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). If the date is a range, it holds the season component of the start date.
...
If the date specification in the \bibfield{origdate} field is a range, this field holds the season component of the end date as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). A blank (but defined) \bibfield{origendseason} component indicates an open ended \bibfield{origdate} range.
...
This field holds the season component of the \bibfield{eventdate} field as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). If the date is a range, it holds the season component of the start date.
...
If the date specification in the \bibfield{eventdate} field is a range, this field holds the season component of the end date as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). A blank (but defined) \bibfield{eventendseason} component indicates an open ended \bibfield{eventdate} range.
...
This field holds the season component of the \bibfield{urldate} field as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). If the date is a range, it holds the season component of the start date.
...
If the date specification in the \bibfield{urldate} field is a range, this field holds the season component of the end date as specified by \acro{EDTF} 5.2.5 (\secref{bib:use:dat}). It contains a season localisation string (\secref{aut:lng:key:seasons}). A blank (but defined) \bibfield{urlendseason} component indicates an open ended \bibfield{urldate} range.
...
\item Given as a per-namelist option \opt{sortnamekeyscheme} in datasources which support this
\item Given as a per-name option \opt{sortnamekeyscheme} in datasources which support this
\end{itemize}
...
\cmditem{if<datepart>dateera}{era}{true}{false}

Expands to \prm{true} if the date <datepart>date (\opt{date}, \opt{urldate}, \opt{eventdate} etc.) has an era specification equal to \prm{era} and \prm{false} otherwise.  The supported \prm{era} strings which \biber determines and passes in the \file{.bbl} are:

\begin{description}
\item[bce] BCE/BC era
\end{description}

All dates are assumed to be CE/AD era by default. This command is useful for determining whether to print the location strings in \secref{aut:lng:key:era}.

\cmditem{ifdateera}{era}{true}{false}

As \cmd{if<datepart>dateera} but for use in \cmd{mkbibdate*} formatting commands (\secref{aut:fmt:lng}) inside which the appropriate \cmd{if<datepart>dateera} command is aliased to this command.

\cmditem{if<datepart>datecirca}{true}{false}

Expands to \prm{true} if the date <datepart>date (\opt{date}, \opt{urldate}, \opt{eventdate} etc.) had a <circa> marker in the source and \prm{false} otherwise.  See \secref{bib:use:dat}. This command is useful for determining whether to print the location strings in \secref{aut:lng:key:circa}.

\cmditem{ifdatecirca}{true}{false}

As \cmd{if<datepart>datecirca} but for use in \cmd{mkbibdate*} formatting commands (\secref{aut:fmt:lng}) inside which the appropriate \cmd{if<datepart>datecirca} command is aliased to this command.

\cmditem{ifdate<datepart>uncertain}{true}{false}

Expands to \prm{true} if the date <datepart>date (\opt{date}, \opt{urldate}, \opt{eventdate} etc.) had an uncertainty marker in the source and \prm{false} otherwise.  See \secref{bib:use:dat}. This command is useful for determining whether to print, for example, a question mark after a year.

\cmditem{ifdateuncertain}{true}{false}

As \cmd{if<datepart>dateuncertain} but for use in \cmd{mkbibdate*} formatting commands (\secref{aut:fmt:lng}) inside which the appropriate \cmd{if<datepart>dateuncertain} command is aliased to this command.
...
\paragraph{Seasons}
\label{aut:lng:key:seasons}

Abbreviation strings for seasons parsed from \acro{EDTF} dates:
...
Similar to \texttt{date} but controls the format of \cmd{printlabeldate}.
...
\item sortnamekeyscheme can be now be set per-namelist and per-name for
  \bibtex datasources\see{aut:ctm:srt}
\item Added \cmd{DeclareDelimcontextAlias}\see{use:fmt:csd}
\item Aligned dates with \acro{EDTF}. Associated tests and localisation strings\see{bib:use:dat}
