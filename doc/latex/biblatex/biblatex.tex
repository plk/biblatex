...
\end{lstlisting}
%
With options \opt{vform=original}, \opt{vlang=russian} and \opt{vtranslang=english}, \biber would output a \file{.bbl} entry like this:
...
\item The \bibfield{labelname} is implicitly \opt{form=original} and \opt{lang=russian}
\item The \opt{form=translated} fields are all implicitly \opt{lang=english} due to the global \opt{vtranslang} setting. Note that \biber has therefore omitted the explicit language setting for the \bibfield{title} field in the data source since it matches \opt{vtranslang}.
\end{itemize}
...
\end{lstlisting}
%
Macros which allow specification of variants take an optional \prm{variantspec} parameter which is a keyval item consisting of a \opt{form=} key option and/or a \opt{lang=} key option.

All macros which allow specific variants to be selected default the \opt{form} option to the \opt{vform} option value, the \opt{lang} option to the \opt{vlang} option value and for \opt{translated} forms, the \opt{lang} option to the \opt{vtranslang} option value. Hence, in the above example, specifying \opt{lang=english} for the translated \bibfield{title} field is not strictly necessary.

If a data source entry contains a variant enabled field in both an unqualified and equivalent qualified variant (given the \opt{vform} and \opt{vlang} option settings), then the last one in the entry is used. For example, given \opt{vform=original} and \opt{vlang=english}:
...
Specifies the \sty{babel}/\sty{polyglossia} language for the variant enabled field specified with \opt{field} if the datasource you are using supports this. If absent, default to the value of the \opt{vlang} option or \opt{vtranslang} if the \opt{form=translated}. See \secref{bib:fld:var} and \secref{use:opt:pre:gen}.
