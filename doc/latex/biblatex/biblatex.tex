...
This option controls whether dates between the dates specified with the \opt{julianstart} and \opt{julianend} options will be converted automatically to the Julian Calendar. Dates so changed will return <true> for the \cmd{ifdatejulian} and \cmd{if$<$datetype$>$datejulian} tests (see \secref{aut:aux:tst}).
