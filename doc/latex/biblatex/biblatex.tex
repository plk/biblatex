...
\end{lstlisting}
%
Notice that the macro \cmd{mkbibnamepatronymic} (as with all \cmd{makebibname*} macros) and the macro \cmd{namepartpatronymic} (as with all \cmd{namepart*} macros) were automatically created by the datamodel <nameparts> constant definition. We would probably like to make the \biber sorting system aware of this new name part and so we could define, for example (see \secref{aut:ctm:srt}):
...
\item Changes for name formats to generalise available name parts\see{aut:bib:fmt} and \see{use:names}\BiberOnlyMark
