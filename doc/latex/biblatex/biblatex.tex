...
The language specific separator to be used for date ranges. Defaults to \cmd{bibdatedash} for all date formats apart from \opt{ymd} which defaults to a slash. The date format option \opt{edtf} is hard-coded to a slash since this is standards compliant format.
...
Modifies a timezone string passed in as the only argument. By default this changes <UTC> to the value of \cmd{bibtimezone}.
