...
This option controls how date era information is printed. <simple> uses \cmd{dateeraprintpre/enddateeraprintpre} to print era information \emph{before} start/end dates. <secular> and <christian> uses \cmd{dateeraprint/enddateeraprint} to print era information \emph{after} the start/end/dates. By default <simple> results in a minus sign before BCE/BC dates and <secular>/<christian> results in the relevant localisation strings like <BCE> or <BC> after BCE/BC dates. See the relevant comments in \secref{use:fmt:fmt} and the localisation strings in \secref{aut:lng:key:era}.

\intitem{dateeraauto}

This option sets the integer number for CE/AD years, below which CE/AD era localisation strings will automatically be added. This option does nothing without \opt{dateera} being set to <secular> or <christian>.
...
labeldate      & labelyear        & extrayear      &  labelname+\\
