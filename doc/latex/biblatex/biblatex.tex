...
In <entry> mode, \biblatex uses the variant information in the field name to potentially switch language settings, depending on the \opt{autolangshort*} options. It is expected that in <entry> mode, there is no more than one variant per field in an entry. A warning will be generated if a field with more than one variant is found in an entry. This mode is intended for automatic language switching for entries where the language/script of fields may not all be the same, as in a French article in an Italian collection. For example, given the following entry in a document typeset with babel/polyglossia main language set to <french> and with \opt{autolangshort} set to <hyphenation> (see the \biber manual for details about this extended variant \file{.bib} format):
...
\biblatex would print the \bibfield{BOOKSUBTITLE}, \bibfield{BOOKTITLE}, \bibfield{PUBLISHER} and \bibfield{SERIES} fields using italian hyphenation. In <entry> mode, the data printing commands:
