...
Date fields such as the default data model dates \bibfield{date}, \bibfield{origdate}, \bibfield{eventdate}, and \bibfield{urldate} support ISO8601v2004 date formats including date ranges. In addition to ISO8601v2004, the \biblatex date format also supports open date ranges and explicit era specifications. You may specify an open ended date range by giving the range separator and omitting the end date (\eg \texttt{YYYY/}). See \tabref{bib:use:tab1} for some examples of valid date specifications and the formatted date automatically generated by \biblatex. The formatted date is language specific and will be adapted automatically. If there is no \bibfield{date} field in an entry, \biblatex will also consider the fields \bibfield{year} and \bibfield{month} for backwards compatibility with traditional \bibtex but this is not encouraged as explicit \bibfield{year} and \bibfield{month} are not parsed for date meta-information markers and are used as-is.. Style authors should note that date fields like \bibfield{date} or \bibfield{origdate} are only available in the \file{bib} file. All dates are parsed and dissected into their components as the \file{bib} file is processed. The date components are made available to styles by way of the special fields discussed in \secref{aut:bbx:fld:dat}. See this section and \tabref{aut:bbx:fld:tab1} on page~\pageref{aut:bbx:fld:tab1} for further information.
...
\biblatex also supports dates before common era (BCE/BC) in the standard ISO8601 negative date format or with explicit era markers. It also supports dates with <circa> markers and uncertain date markers. Such dates set internal markers which can be tested for so that appropriate localised markers (such as \opt{circa} or \opt{beforecommonera}) can be inserted. The table shows formats which use appropriate tests and formatting. See the date meta-information tests in \secref{aut:aux:tst} and the localisation strings in in \secref{aut:lng:key:era} and \secref{aut:lng:key:circa}. See also the \file{96-dates.tex} example file for a complete example of the tests and localisation strings use.

To summarise, the supported explicit meta-information markers supported in date fields are:

\begin{itemize}
\item Era marker suffix <BC>, <BCE>, <AD> and <CE>. Space prefix for the marker is optional. The <AD/CE> markers are not strictly necessary since all dates are assumed to be in this era unless otherwise determined or explicitly specified.
\item <circa> marker <c>, <ca> or <circa> prefix. Dot and/or space suffix for the marker is optional.
\item EDTF\footnote{\url{https://www.loc.gov/standards/datetime/pre-submission.html}} approximation (circa) marker <\textasciitilde> suffix. Space prefix for the marker is optional.
\item Uncertainty marker suffix <?>. Space prefix for the marker is optional.
\end{itemize}
%
...
\cmidrule{1-1}\cmidrule(l){2-2}\cmidrule(l){3-3}
-876			  & 877 BCE			     & using \cmd{ifdateera} test and \opt{beforecommonera} localisation string\\
877 BC			  & 877 BCE			   & Supported explicit eras are <BC>, <BCE>, <AD>, <CE>. Space before is optional\\
-0877/-0866 & 878 BC--867 BC & using \cmd{ifdateera} test and \opt{beforechrist} localisation string\\
124BC/122BC & 124 BC--122 BC & \\
2004 CE & 2004 CE & using \opt{commonera} localisation string\\
-034302 & 344-02 BCE & \\
-0343-02 & 344-02 BCE & \\
-03430203 & 344-02-03 BCE & \\
343-02-03BC & 343-02-03 BCE & \\
-03430203/-03400405 & 344-02-03--341-04-05 BCE & \\
c 1723 & c. 1723 & using \cmd{ifdatecirca} test\\
1723\textasciitilde & c. 1723 & using \cmd{ifdatecirca} test\\
ca. 1723 CE & c. 1723 & using \cmd{ifdatecirca} test\\
circa 1723 CE & c. 1723 & using \cmd{ifdatecirca} test\\
circa. 1723-02 BC & c. 1723-02 BC & using \cmd{ifdatecirca}, \cmd{ifdateera} tests and \opt{beforechrist} localisation string\\
...
\item Enhanced ISO8601v2004 dates with meta-information, associated tests and localisation strings\see{bib:use:dat}
