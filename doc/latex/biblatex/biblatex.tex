...
\boolitem[false]{minsortnamesstrict}

If this option is enabled, the visible list of names (which is the \opt{minsortnames} value) used to determine sorting will ignore any truncation. This means that the name lists:
...
\end{lstlisting}
%
will sort exactly the same. The default setting of this option always sorts in the order shown in the example, that is, by default, truncated names lists always sort after any name lists identical to the point of truncation.
...
\optitem[strong]{uniqueliststrength}{\opt{strong}, \opt{weak}}

When generating the counters for \bibfield{extraname} and \bibfield{extradate}, by default \biber will take account of name list truncation due to the \opt{uniquelist} settings (and any explicit <and others> in the data source). This can be weakened to ignore name truncations when generating the \bibfield{extraname} and \bibfield{extradate} counters. The possible choices are:

\begin{valuelist}
\item[strong] Take account of name list truncation when generating \bibfield{extraname} and \bibfield{extradate}.
\item[weak] Ignore name list truncation when generating \bibfield{extraname} and \bibfield{extradate}.
\end{valuelist}
%
Note that the \opt{uniqueliststrength} option will also affect the \bibfield{extradate} and \bibfield{extraname} fields. See \secref{aut:cav:amb} for further details and practical examples.
...
\end{ltxexample}
%
This can be customised to add more name parts to deal with things like patronymics (see the example file \file{93-nameparts.tex}). Naturally this needs an extended name format for data sources. \biblatexml (\secref{apx:biblatexml}) handles this natively and there is an extended name format which can handle custom nameparts when using \biber (see \biber documentation).
...
With \kvopt{uniqueliststrength}{weak}, it is possible to further disambiguate name lists which would otherwise be treated as unique due to their truncations being unique but which nevertheless might appear confusing. Consider:
...
\end{lstlisting}
%
With \kvopt{maxnames}{3}, \kvopt{minnames}{1}, \kvopt{uniqueliststrength}{strong}, we would get:
...
\end{lstlisting}
%
Whereas with \kvopt{maxnames}{3}, \kvopt{minnames}{1}, \kvopt{uniqueliststrength}{weak}, we would get:
...
timezones	&\+&\+&\_&\_&\_&\_&\_\\
uniquelist	&\+&\+&\_&\+&\+&\_&\_\\
uniqueliststrength	&\+&\_&\_&\_&\_&\_&\_\\
uniquename	&\+&\+&\_&\+&\+&\_&\_\\
...
\begin{changelog}
\begin{release}{3.13}{2019}
\item Added \opt{minsortnamesstrict}\see{use:opt:pre:gen}
\item Added \opt{uniqueliststrength}\see{use:opt:pre:int}
