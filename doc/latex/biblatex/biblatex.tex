...
defined. Global, per-type and per-entry options called <use$<$name$>$>are automatically created. Unlike \opt{useauthor}, \opt{useeditor} and \opt{usetranslator}, any other \opt{use$<$name$>$} options may not be set as a package option in \cmd{usepackage} but may be set with \cmd{ExecuteBibliographyOptions}. For example, after defining a new name list \bibfield{proofreader} field in the data model (see \secref{aut:ctm:dm}, you can enable its use in sorting/labelling by putting this in the preamble:
