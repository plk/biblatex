...
This option sets the global sorting locale. Every sorting scheme inherits this locale if none is specified using the \prm{locale} option to \cmd{printbibliography}. Setting this to \opt{auto} requests that it be set to the Babel/Polyglossia main document language identifier, if these packages are used and \texttt{en\_US} otherwise. \biber will map Babel/Polyglossia language identifiers into sensible locale identifiers (see the \biber documentation). You can therefore specify either a normal locale identifier like \texttt{de\_DE_phonebook}, \texttt{es_ES} or one of the supported Babel/Polglossia language identifiers if the mapping \biber makes of this is fine for you.
...
\cmditem{locale}{\prm{locale}}

Specifies the locale for the sorting scheme. This can be used to tailor a sort to a language whose sort rules are not covered by the standard Unicode Collation Algorithm. You can specify a Babel/Polyglossia language identifier (which \biber will map into a suitable locale, see the \biber documentation) or a real language locale identifier like \texttt{de\_DE\_phonebook}. If not present, the \biblatex global sorting locale setting is inherited from the \opt{sortlocale} option.
