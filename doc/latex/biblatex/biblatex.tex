...
\valitem{sortingnamekey}{name}

Specify a sorting name key scheme defined previously with \cmd{DeclareSortingNamekey}. This scheme is used to construct sorting keys for names inside the context.
...
Declares the \prm{name} as a datamodel constant with definition \prm{constantdef}. Such constants are typically used internally by the \biber backend but may be of use to style authors etc.
...
\cmditem{DeclareSortingNamekey}[schemename]{specification}\BiberOnlyMark

Defines how the sorting keys for names in a name list are constructed. The sorting key construction scheme so defined is called \prm{schemename} which defaults to «global» if this optional parameter is absent. When constructing the sorting key for a name list, a sorting key for each name in the list is constructed and the key for each name is formed into an ordered key list with a special internal separator. The point of this option is to accommodate languages or situations where sorting of names needs to be customised (for example, Icelandic names are sometimes sorted by given names rather than by family names). By default the name key is formed according the following \prm{specification}:

\begin{lstlisting}[style=latex]{}
\DeclareSortingNamekey{
  \keypart{
...
\prm{part} is an ordered list of of \cmd{namepart} and \cmd{literal} specifications which are concatenated together when to construct a part of the name sorting key.
...
\item Added \cmd{DeclareSortingNamekey}\see{\label{aut:ctm:srt}}
