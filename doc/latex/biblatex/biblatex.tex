...
\optitem[HH]{$<$datetype$>$time}{\opt{hh}, \opt{HH}, \opt{HHcomp}}
...
\optitem{alltimes}{\opt{hh}, \opt{HH}, \opt{HHcomp}}

Sets the option for all times in the datamodel to the same value. The date fields supporting time parts in the default data model are \bibfield{date}, \bibfield{origdate}, \bibfield{eventdate} and \bibfield{urldate}.
...
This command prints the time of the entry, as specified in the \bibfield{date} field (see \secref{bib:use:dat}). The time format is controlled by the package option \opt{time} from \secref{use:opt:pre:gen}. Additional formatting (fonts etc.) may be applied by adjusting the field format \texttt{time} (\secref{aut:fmt:ich}). Note that this command interfaces with the punctuation tracker. There is no need to wrap it in a \cmd{printtext} command.
...
A special formatting directive which controls the format of \cmd{printtime} (\secref{aut:bib:dat}). Note that the time format (HH/hh etc.) is controlled by the package option \opt{time} from \secref{use:opt:pre:gen}. This formatting directive only controls additional formatting such as fonts etc.
