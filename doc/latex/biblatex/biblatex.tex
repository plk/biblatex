...
\tabref{bib:use:tab2} shows formats which use appropriate tests and formatting. See the date meta-information tests in \secref{aut:aux:tst} and the localisation strings in in \secref{aut:lng:key:era} and \secref{aut:lng:key:circa}. See also the \file{96-dates.tex} example file for complete examples of the tests and localisation strings use.
...
\optitem[comp]{<datetype>date}{\opt{year}, \opt{short}, \opt{long}, \opt{terse}, \opt{comp}, \opt{edtf}}
...
\optitem[simple]{dateera}{\opt{simple}, \opt{secular}, \opt{christian}}

This option controls how date era information is printed. <simple> uses \cmd{dateeraprintpre/enddateeraprintpre} to print era information \emph{before} start/end dates. <secular> and <christian> uses \cmd{dateeraprint/enddateeraprint} to print era information \emph{after} the start/end/dates. By default <simple> results in a minus sign before BCE/BC dates and <secular> and <christian> results in the relevant localisation strings like <BCE> or <BC> after BCE/BC dates. See the relevant commends in \secref{use:fmt:fmt} and the localisation strings in \secref{aut:lng:key:era}.
...
Whether or not to provide the special fields \bibfield{labelyear}, \bibfield{labelmonth}, \bibfield{labelday}, \bibfield{labelendyear}, \bibfield{labelendmonth}, \bibfield{labelendday} and \bibfield{extrayear}, see \secref{aut:bbx:fld} for details. 
...
Whether or not to provide the data required by the \cmd{ifuniquework} test, see \secref{aut:aux:tst} for details.
This option is also settable on a per-type basis.
...
labeldate      & label(end)(year,month,day)  & extrayear      &  labelname+labelyear\\
...
\end{itemize}
%
For example, if this option is enabled, \biblatex precedes the last name with the prefix---Ludwig van Beethoven would be cited as «van Beethoven» and alphabetized as «Van Beethoven, Ludwig van». If this option is disabled (the default), he is cited as «Beethoven» and alphabetized as «Beethoven, Ludwig van» instead.
...
Prints date uncertainty information when the global option \opt{datecirca} is enabled. By default, prints the <circa> localised term (\secref{aut:lng:key:circa}) and the \opt{datecircadelim} delimiter.

\csitem{dateeraprint}{yearfield}
Prints date era information when the global option \opt{dateera} is set to <secular> or <christian>. By default, prints the \opt{dateeradelim} delimiter and the appropriate localised era term (\secref{aut:lng:key:era}). If the \opt{dateeraauto} option is set, then the passed \prm{yearfield} (which is the name of a year field such as <year>, <origyear>, <endeventyear> etc.) is tested to see if its value is earlier than the \opt{dateeraauto} threshold and if so, then the CE/AD localisation will be output too. By default, when \opt{dateeraauto} is not used, no CE/AD localisation strings are output for CE/AD dates.

\csitem{enddateeraprint}{yearfield}
As \cmd{dateeraprint} but used after end dates in a date range.

\csitem{dateeraprintpre}
Prints date era information when the global option \opt{dateera} is set to <simple>. By default, prints \opt{bibdataeraprefix}.

\csitem{enddateeraprintpre}
As \cmd{dateeraprintpre} but used before end dates in a date range.
...
The language specific marker which is printed as a prefix to BCE/BC dates when the option \opt{dateera} is set to <simple>. Defaults to a hyphen.
...
The month of the date field selected by \cmd{DeclareLabeldate} (\secref{aut:ctm:fld}), or the \bibfield{month} field for use in author-year labels. Note that the \bibfield{labelmonth} field needs to be requested with the package option \opt{labeldate}, see \secref{use:opt:pre:int} for details.
...
The month of the date field selected by \cmd{DeclareLabeldate} (\secref{aut:ctm:fld}) for use in author-year labels. Note that the \bibfield{labelday} field needs to be requested with the package option \opt{labeldate}, see \secref{use:opt:pre:int} for details.
...
Expands to \prm{true} if the date <datetype>date (\opt{date}, \opt{urldate}, \opt{eventdate} etc.) had a <circa> marker in the source and \prm{false} otherwise.  See \secref{bib:use:dat}. This command is useful for determining whether to print the location strings in \secref{aut:lng:key:circa}.
...
\item Changes \cmd{DeclareStyleSourcemap} so that it can be used multiple times\see{aut:ctm:map}
