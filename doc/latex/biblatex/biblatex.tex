...
\valitem{prefixnumbers}{string}\BibTeXOnlyMark 

This option applies to numerical citation\slash bibliography styles only and requires that the \opt{defernumbers} option from \secref{use:opt:pre:gen} be enabled globally. Setting this option will implicitly enable \opt{resetnumbers} for the bibliography. The option assigns the \prm{string} as a prefix to all entries in the reference context. For example, if the \prm{string} is \texttt{A}, the numerical labels printed will be \texttt{[A1]}, \texttt{[A2]}, \texttt{[A3]}, etc. This is useful for subdivided numerical bibliographies where each subbibliography uses a different prefix. The \prm{string} is available to styles in the \bibfield{prefixnumber} field of all affected entries. See \secref{aut:bbx:fld:lab} for details. With \biber as the backend, this option has been moved to \cmd{newrefcontext} to allow greater flexibility when citing entries in multiple bibliographies with different prefices, see \secref{use:bib:context}.
...
\subsubsection{Reference Contexts}\BiberOnlyMark
...
\begin{itemize}
 ...
 \item A string prefix for numeric citation schemes
\end{itemize}
%
...
\end{ltxsyntax}
These commands automate putting citations into refcontexts when the default behaviour is not sufficient. The default behaviour is that the data for a citation is drawn from the refcontext of the last bibliography in which it was printed. To override this behaviour, instead of manually wrapping citation commands in \env{refcontext} environments, which might be error-prone and tedious, you can register a comma"=separated list of \prm{keywords}, \prm{categories} or \prm{entrykeys} which, respectively, make the entries with any of the specified keywords, entries in any of the specified categories (see \secref{use:use:div}) or entries with any of the specified citation keys be cited in a particular refcontext specified by the \prm{refcontext key/values} which are parsed as the per the corresponding \env{refcontext} options. Such refcontext auto-assignments are specific to the current refsection. You may specify the same citation key in any of these commands but be aware that assignment is done in the order \prm{keywords}, \prm{categories}, \prm{entrykeys} with the later specifications overriding the earlier. An example:
...
\end{ltxexample}
%
which is not so convenient since such specification of refcontexts for citations requires manual insertion of refcontext changing commands wherever the citations occur. You can always locally override these global refcontext assignments to entrykeys in citations by using explicit \env{refcontext} environments or \cmd{newrefcontext}. The unstarred commands force the specified citation refcontext for an entry even if it is cited within an explicit \env{refcontext} environment (\secref{use:bib:context}). The unstarred variants are weaker and do not do this---any explicit \env{refcontext} will take precedence and the citation will draw its data from the refcontext specified by the \env{refcontext} environment.
...
Set the delimiter context to \prm{context}. This assignment is global. Saves any current delimiter context before setting it to \prm{context} so that it can be restored by \cmd{restoredelimcontext}.

\cmditem{restoredelimcontext}

Restores the saved context saved by \cmd{delimcontext}.
...
\item[bib] Inside a bibliography begun with \cmd{printbibliography}
\item[biblist] Inside a bibliography list begun with \cmd{printbiblist}\BiberOnlyMark
\item[shorthands] Inside a shorthand list begun with \cmd{printshorthands}\BibTeXOnlyMark
...
\end{ltxexample}
%
Formatting directives for names are similar to those for literal lists. They differ between the \biber\ and \bibtex\ backends.

\paragraph{Name Formats with \biber}\BiberOnlyMark
...
\paragraph{Name Formats with \bibtex}\BibTeXOnlyMark 
...
Note that with \biber\ it is possible to define new date fields in the datamodel which behave exactly like the default data model date fields described in this section.\BiberOnlyMark
...
The syntax of \cmd{DeclareNameFormat} differs between the \biber\ and \bibtex\ backends.

\paragraph{\biber}\BiberOnlyMark
...
\paragraph{\bibtex}\BibTeXOnlyMark
...
\subsubsection{Datasource Sets}\BiberOnlyMark
\label{aut:ctm:dsets}
...
Intended for name format tests to test if nameparts contain anything. Depending on the backend used, nameparts can be passed in to name formats as strings (\bibtex) or macros (\biber). \prm{arg} must be a single macro or a string.
...
\item New starred variants of the \cmd{assignrefcontext*} macros\see{use:bib:context}\BiberOnlyMark
\item New context-sensitive delimiter interface\see{use:fmt:csd}
\item Moved \opt{prefixnumbers} option to \cmd{newrefcontext} and renamed to \opt{labelprefix}\BiberOnlyMark
\item Added \cmd{DeclareDatafieldSet}\see{aut:ctm:dsets}
\end{release}

\begin{release}{3.3}{2016-03-01}
\item New macros for auto-assignment of refcontexts\see{use:bib:context}\BiberOnlyMark
