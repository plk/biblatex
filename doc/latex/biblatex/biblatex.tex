...
For use in \cmd{mkbibdate*} formatting commands (\secref{aut:fmt:lng}). Expands to \prm{true} if the date type (\opt{date}, \opt{urldate}, \opt{eventdate} etc.) being formatted has an era specification equal to \prm{era}. The supported \prm{era} strings which \biber determines and passes in the \file{.bbl} are:
...
For use in \cmd{mkbibdate*} formatting commands (\secref{aut:fmt:lng}). Expands to \prm{true} if 
the date type (\opt{date}, \opt{urldate}, \opt{eventdate} etc.) had a <circa> marker in the source. See \secref{bib:use:dat}. This command is useful for determining whether to print the location strings in \secref{aut:lng:key:circa}.

\cmditem{ifdateuncertain}{true}{false}

For use in \cmd{mkbibdate*} formatting commands (\secref{aut:fmt:lng}). Expands to \prm{true} if 
the date type (\opt{date}, \opt{urldate}, \opt{eventdate} etc.) had an uncertainty marker in the source. See \secref{bib:use:dat}.
