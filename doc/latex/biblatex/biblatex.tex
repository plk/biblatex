...
This option controls how date era information is printed. <astronomical> uses \cmd{dateeraprintpre/enddateeraprintpre} to print era information \emph{before} start/end dates. <secular> and <christian> uses \cmd{dateeraprint/enddateeraprint} to print era information \emph{after} the start/end/dates. By default <astronomical> results in a minus sign before BCE/BC dates and <secular>/<christian> results in the relevant localisation strings like <BCE> or <BC> after BCE/BC dates. See the relevant comments in \secref{use:fmt:fmt} and the localisation strings in \secref{aut:lng:key:dt}.
...
\optitem[24h]{timelabel}{\opt{12h}, \opt{24h}, \opt{24hcomp}}
...
singletitle & \cmd{ifsingletitle} & labelname or labeltitle\\
...
Prints date era information when the global option \opt{dateera} is set to <astronomical>. By default, prints \opt{bibdataeraprefix}.
...
The language specific marker which is printed as a prefix to BCE/BC dates when the option \opt{dateera} is set to <astronomical>. Defaults to a hyphen.
...
\valitem{suppress}{csv list of uniqueness options}

This option takes a comma-separated list of one of more of <singletitle>, <uniquetitle> and/or <uniquework>. The purpose of this option is to suppress tracking information for these three options when the field which would trigger the tracking (\tabref{use:opt:wu})is inherited. An example---Suppose that you have several \bibtype{book} entries which all crossref a \bibtype{mvbook} from which they get their \bibfield{author} field. You might reasonably want the \cmd{ifsingletitle} test to return <true> for this author as their only <work> is the \bibtype{mvbook}. Similar comments would apply to situations involving the \cmd{ifuniquetitle} and \cmd{ifuniquework} tests. The \opt{suppress} option lists which of these should have their tracking information suppressed when the fields which would trigger them are inherited. The idea is that the presence of an inherited field does not contribute towards the determination of whether some combination of name/title is unique in the bibliographic data. For example, this would suppress \opt{singletitle} and \opt{uniquetitle} tracking by default:

\begin{ltxexample}
\DefaultInheritance{suppress={singletitle,uniquetitle}}
\end{ltxexample}
%
Of course, the suppression of tracking does nothing if the fields inherited do not play a role in tracking. Only the fields listed in \tabref{use:opt:wu} are relevant to this option.
...
As the \opt{suppress} option on \cmd{DefaultInheritance} explained above. When set here, it takes precedence over any global options set with \cmd{DefaultInheritance}. For example, this would suppress \opt{singletitle} and \opt{uniquetitle} tracking for a \bibtype{book} inheriting from a \bibtype{mvbook}.

\begin{ltxexample}
\DeclareDataInheritance[suppress={singletitle,uniquetitle}]{mvbook}{book}{<<...>>}
\end{ltxexample}
...
\item Added full \acr{EDTF} Levels 0 and 1 compliance for parsing and print times\see{bib:use:dat}
