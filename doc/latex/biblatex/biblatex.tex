...
\cmditem{assignrefcontextkeys}[refcontext key/values]{entrykeys}
\cmditem{assignrefcontextcat}[refcontext key/values]{category}

These commands automates putting cites into refcontexts. Instead of wrapping citation commands in \env{refcontext} environments, which might be error-prone and tedious, you can register a comma"=separated list of \prm{entrykeys} or \prm{categories} which makes the entries (or entries in the categories) always be cited from a particular refcontext specified by the \prm{refcontext key/values} which are parsed exactly as the per \env{refcontext} options.
\end{ltxsyntax}
