...
\hypersetup{%
  ...
  pdfauthor={},
  pdfkeywords={tex, latex, bibtex, biber, bibliography, references, citation}}
...
This package provides advanced bibliographic facilities for use with \latex in conjunction with \biber. The package is a complete reimplementation of the bibliographic facilities provided by \latex. It redesigns the way in which \latex interacts with the bibliographic backend at a fairly fundamental level. With \sty{biblatex}, instead of being implemented in \file{bst} files, the formatting of the bibliography is entirely controlled by \latex macros, hence the name \sty{biblatex}. Good working knowledge in \latex should be sufficient to design new bibliography and citation styles. This package also supports subdivided bibliographies, multiple bibliographies within one document with different sorting, and separate lists of bibliographic shorthands with their own sorting. Bibliographies may be subdivided into parts and\slash or segmented by topics. Just like the bibliography styles, all citation commands may be freely defined. The package is completely localized and can interface with the \sty{babel} package. Please refer to \tabref{bib:fld:tab1} for a list of languages currently supported by this package.
...
\item[Biber]
\biber is the dedicated backend of \sty{biblatex}. It performs all of the sorting, label generation, uniqueness information resolution, data source parsing and quite a lot more besides. It was written specifically for \sty{biblatex} as a \bibtex replacement. It supports full \utf.
...
This part of the manual documents the \bibtex file format interface of this package. Since version 2, \sty{biblatex} does not use \bibtex (the program) but supports \bibtex (the file format) fully. It is easy to switch from \latex and \bibtex to \sty{biblatex} and \biber as \biber uses a \bibtex compatible library to read \bibtex format files.
...
The fields listed in this section are the regular ones holding printable data. The name on the left is the name of the field, as used by both the \bibtex file format and \sty{biblatex}. The \sty{biblatex} data type is given to the right of the name. See \secref{bib:fld:typ} for explanation of the various data types.
...
\fielditem{ids}{comma"=separated values}

Citation key aliases for the main citation key. An entry may be cited by any of its aliases and \sty{biblatex} will treat the citation as if it had used the primary citation key. This is to aid users who change their citation keys but have legacy documents which use older keys for the same entry.
...
\fielditem{related}{comma"=separated values}
...
\fielditem{xdata}{comma-separated list of entry keys}
...
\paragraph{The \bibfield{crossref} field}
\label{bib:cav:ref:bbr}

With \biber, the limitations of \bibtex's \bibfield{crossref} field belong to the past. \biber features a highly customizable cross-referencing mechanism with flexible data inheritance rules. Entries are specified in a natural way:
...
\item Latin\,9 encoding (\acr{ISO}-8859-15) in both the \file{tex} and the \file{bib} file with \pdftex or traditional \tex:
...
The default sorting order of the bibliography. When \cmd{printbibliography} is used without the \opt{sorting} option, it defaults to this setting. Unless stated otherwise, the entries are sorted in ascending order. The following choices are available by default:
...
This option corresponds to \biber's |--sortlocale| command-line option. If set, it specifies the locale to be used for sorting. While \sty{biblatex} provides no default value for this option, \biber will inherit the locale settings from the environment if no locale has been specified explicitly.
...
Specifies the encoding of the \file{tex} file. This option affects the data transfered from the backend to \sty{biblatex}. This corresponds to \biber's |--bblencoding| option. The following choices are available:
...
Specifies the encoding of the \file{bib} files. This corresponds to \biber's |--bibencoding| option. The following choices are available:
...
By default, \sty{biblatex} assumes that the \file{tex} file and the \file{bib} file use the same encoding (\kvopt{bibencoding}{auto}). See \secref{bib:cav:enc} for further instructions.
...
By default, \sty{biblatex} will report warnings issued by \biber concerning the data in the \file{bib} file as \latex warnings. Use this option to suppress such warnings.

\intitem[2]{mincrossrefs}

Sets the minimum number of cross references to \prm{integer} when requesting a \biber run. If an entry which is cross-referenced by other entries in the \file{bib} file hits this threshold, it is included in the bibliography even if it has not been cited explicitly. This is a standard feature of \bibtex (also \biber) and not specific to \sty{biblatex}. See the description of the \bibfield{crossref} field in \secref{bib:fld:spc} for further information. This option also affects the handling of the \bibfield{xref} field. See the field description in \secref{bib:fld:spc} as well as \secref{bib:cav:ref} for details.
...
When enabled, sorting names will only use their initials. This is separate from \opt{firstinits} in case users want to show only inits but sort on full names, for example.

\boolitem[false]{terseinits}

This option is a high-level control for the format of initials generated by \sty{biblatex}. If enabled, initals are rendered using a terse format without dots and spaces. For example, the initials of Donald Ervin Knuth would be rendered as <D.~E.> by default, and as <DE> if this option is enabled. The option works by redefining some macros which control the format of initials. See \secref{use:cav:nam} for details on how this can be customised for more fine-grained control.

\boolitem[false]{labelalpha}

Whether or not to provide the special fields \bibfield{labelalpha} and \bibfield{extraalpha}, see \secref{aut:bbx:fld} for details. This option is also settable on a per-type basis. See also \opt{maxalphanames} and \opt{minalphanames}.
...
Whether or not to provide the special field \bibfield{labelnumber}, see \secref{aut:bbx:fld} for details. This option is also settable on a per-type basis.

\boolitem[false]{labelyear}

Whether or not to provide the special fields \bibfield{labelyear} and \bibfield{extrayear}, see \secref{aut:bbx:fld} for details. This option is also settable on a per-type basis.

\boolitem[false]{singletitle}

Whether or not to provide the data required by the \cmd{ifsingletitle} test, see \secref{aut:aux:tst} for details. This option is also settable on a per-type basis.
...
Whether the \bibfield{author} is used in labels and considered during sorting. This may be useful if an entry includes an \bibfield{author} field but is usually not cited by author for some reason. Setting \kvopt{useauthor}{false} does not mean that the \bibfield{author} is ignored completely. It means that the \bibfield{author} is not used in labels and ignored during sorting. The entry will then be alphabetized by \bibfield{editor} or \bibfield{title}. With the standard styles, the \bibfield{author} is printed after the title in this case. See also \secref{use:srt}. This option is also settable on a per-type basis.

\boolitem[true]{useeditor}

Whether the \bibfield{editor} replaces a missing \bibfield{author} in labels and during sorting. This may be useful if an entry includes an \bibfield{editor} field but is usually not cited by editor. Setting \kvopt{useeditor}{false} does not mean that the \bibfield{editor} is ignored completely. It means that the \bibfield{editor} does not replace a missing \bibfield{author} in labels and during sorting. The entry will then be alphabetized by \bibfield{title}. With the standard styles, the \bibfield{editor} is printed after the title in this case. See also \secref{use:srt}. This option is also settable on a per-type basis.
...
\end{lstlisting}
%
With the standard styles, the \bibfield{translator} is printed after the title by default. See also \secref{use:srt}. This option is also settable on a per-type basis.

\boolitem[false]{useprefix}

Whether the name prefix (von, van, of, da, de, della, etc.) is considered when printing the last name in citations. This also affects the sorting and formatting of the bibliography as well as the generation of certain types of labels. If this option is enabled, \sty{biblatex} always precedes the last name with the prefix. For example, Ludwig van Beethoven would be cited as «Beethoven» and alphabetized as «Beethoven, Ludwig van» by default. If this option is enabled, he is cited as «van Beethoven» and alphabetized as «Van Beethoven, Ludwig» instead. This option is also settable on a per-type basis.
...
If this option is enabled, the entry is excluded from the list of shorthands. It is still included in the bibliography and it may also be cited by shorthand. This option is also settable on a per-type basis.
...
Setting this option is equivalent to \opt{skipbib}, \opt{skiplos}, and \opt{skiplab}. It is not required for normal operation. Use it with care. This option is also settable on a per-type basis.
...
\begin{itemize}
\item If the localization string «\texttt{reprintof}» is defined, it is printed, then
...
\item[bibtex] \bibtex file format.
...
\item[zoterordfxml] Zotero \acr{RDF}/\acr{XML} format. Support for this format is experimental. Refer to the \biber manual for details.

\item[endnotexml] EndNote \acr{XML} format. Support for this format is experimental. Refer to the \biber manual for details.
...
Use this command in \file{bib} files if \sty{biblatex} gets the <and> before a certain name wrong. As its name suggests, it will enforce <e>. This command must be used in a special way to prevent confusing \biber's \bibtex file format parsing engine. Here is an example:
...
Whenever a reference to a work which has not been cited before is added, this procedure must be repeated. This is also the case if the last reference to a work which has been cited before is removed because some citation labels may change in this case. In contrast to traditional \bibtex, there is normally no need to run \bin{latex} twice after running \bin{biber} as far as the handling of bibliographic data is concerned.\footnote{That is, unless the \opt{defernumbers} package option is enabled. This option requires two \latex runs after the data has been exported to the \file{bbl} file by \biber.} Note that this only applies to the most basic case. Using the \bibfield{xref} field or the \bibfield{entryset} field may require an additional \latex cycle. Some other facilities provided by \sty{biblatex} may also require an additional \bin{latex} run to get certain references and the page tracking right. In this case, the usual warning messages such as «There were undefined references» and «Label(s) may have changed. Rerun to get cross-references right» will be printed. \biber only ever needs one pass to generate all information for the \path{.bbl}.
...
Static entry sets are defined in the \file{bib} file like any other entry. Defining an entry set is as simple as adding an entry of type \bibtype{set}. The entry has an \bibfield{entryset} field defining the members of the set as a comma"=separated list of entry keys:
...
\end{lstlisting}
%
When using \acr{JSTOR}'s export feature to export citations in \bibtex file format, \acr{JSTOR} uses the \bibfield{url} field by default (where the \prm{number} is a unique and stable identifier):
...
The entry key of an item in the \file{bib} file. This is the string used by \sty{biblatex} and \biber to identify an entry in the \file{bib} file. 
...
\item[relatedstring] If an entry contains this field, then the value of the field is printed literally before the date from the related entries. If both \bibfield{relatedtype} and \bibfield{relatedstring} are present in an entry, \bibfield{relatedstring} is used for the pre-data string (but \bibfield{relatedtype} is still used to determine the macro and format to use when printing the data).
\end{keymarglist}
...
\end{lstlisting}
%
Here, we have used the hexadecimal escape sequence <|\x20|> in place of literal spaces.
%
\begin{lstlisting}[style=latex]{}
\DeclareSourcemap{
  \maps[datatype=bibtex]{
    \map[overwrite]{
       ...
       \step[fieldsource=shortauthor, origfieldval]
       \step[fieldsource=sortname, origfieldval]
...
Agassi, Connors, Lendl  [ACon+]
Agassi, Courier, Laver  [ACou+]
...
\cmditem{DeclareSortingScheme}{name}{specification}
...
Defines regular expressions to strip from particular fields or types of fields when sorting. The \prm{specification} is an undelimited list of \cmd{nosort} directives which specify the regular expressions to remove from particular fields or type of field. Spaces, tabs, nd line endings may be used freely to visually arrange the \prm{specification}. Blank lines are not permissible. This command may only be used in the preamble.

\cmditem{nosort}{field or field type}{regexp}

Any number of \cmd{nosort} commands can be given each of which specifies to remove the \prm{regexp} from the \prm{field} or \prm{field type}. A \prm{field type} is simple a convenience grouping of semantically similar fields which you might want to remove information from for sorting as a group. Table \ref{aut:nosort} shows the available field types. Since regular expressions usually contain all sort of special characters, it is best to enclose them in the provided \cmd{regexp} macro as show as this will pass the expression through to \biber correctly.
...
\end{lstlisting}
%
This \biber default strips diacritics and prefices from names when sorting. Suppose you wanted to ignore «The» at the beginning of a \bibfield{title} field when sorting:
...
\cmditem{DeclareLabelyear}[entrytype, \dots]{specification}
...
\csitem{ResetDataInheritance} Clears all inheritance rules defined with \cmd{DeclareDataInheritance}. This command may only be used in the preamble.
...
\end{lstlisting}
%
In \file{bib} files designed with traditional \bibtex file format in mind, it has been fairly common to only wrap single letters in braces to prevent case"=changing:
...
Even though only \biber is supported as a data backend now, the revision history still contains the markers showing which new features required \biber as this is interesting historical information. It shows, for example, that most of the major recent features were «Biber only» and shows an aspect of the motivation to deprecate \bibtex as a backend.

\begin{changelog}

\begin{release}{2.0}{2012-05-31}
\item Misc bug fixes
\item Removed field \bibfield{reprinttitle}\see{use:rel}
\item Removed all \bibtex code and docs.
