...
\choitem{context}{none, init, full, fullonly}

The \prm{namepart} will be disambiguated at most by information at the given <context>. If this option is not present then the default is inferred from the \opt{uniquename} package option setting (see \secref{use:opt:wu}). The <context> option is ignored for \prm{namepart}s which have the <base> option set to <true> since it is these nameparts which are being disambiguated by the <context> of the non-base \prm{namepart}s and therefore <context> does not apply.
...
\item[full]~Use initials and if necessary the full \prm{namepart} to perform name disambiguation
\item[fullonly]~Use only the full \prm{namepart} to perform name
...
\item Added new \opt{context} option to \cmd{DeclareUniquenameTemplate}\see{aut:cav:amb}
