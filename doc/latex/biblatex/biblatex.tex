...
There are two <modes> for multiscript support in \biblatex. The mode is a global setting controlled by the option \opt{msmode }which can also be set per-entry. The two modes available are <entry> mode and <db> mode. The default mode is <entry> mode.
