...
\newrobustcmd*{\VE}{%
  \textcolor{spot}{\margnotefont Variant enabled}}
\newrobustcmd*{\VEMark}{%
  \leavevmode\marginpar{\VE}}
...
To support multilingual bibliographies, data fields can have one or more \emph{variants}. A variant is a combination of a \emph{form} and a \emph{language}. This information is used by \biber to recognise valid forms but mostly it is for use by styles which might decide to present different form or language variants of a field in addition to the most common form or variant (displaying a translated form of a field in addition to the original field, for example). Another use case is for styles which need to format certain form or language variants of a field in a particular way, for example names in languages which default always to <Last First>. By default, there are four supported forms:
...
The list of supported forms can be customised using \cmd{DeclareVariantforms}:

\begin{ltxsyntax}

\cmditem{DeclareVariantforms}{forms}\BiberOnlyMark

Declares the (csv format) list of valid variant forms. \biber will ignore any field form variants not specified by this macro.

\end{ltxsyntax}

If there is no \cmd{DeclareVariantforms} specification, the default is

\begin{lstlisting}[style=latex]
\DeclareVariantforms{original,translated,romanised,uniform}
\end{lstlisting}
%
If you wish to customise this, be sure to keep these defaults in your specification---they must be present.

The supported languages for a variant are any of the supported Babel/Polyglossia language identifiers and these are used to switch languages on a per-variant basis, if required. \biber supports specification of field variants with an entry in a backwards compatible manner so that existing \file{.bib} databases, do not need to be modified. The format for this in \bibtex format bibliography databases is described in the \biber documentation. A variant enabled field given in the database without any variant specification defaults to the values of the \opt{vform} and \opt{vlang} package/per-entry options. When the form «translated» is selected, the language defaults to the value of the \opt{vtranslang} package/per-entry option. When writing the \file{.bbl}, \biber will set the \opt{form} and \opt{lang} options on the field, copying these from the data source. See \secref{use:opt:pre:gen}.

There are two <modes> for multiscript support in \biblatex. The mode is a global setting controlled by the option \opt{msmode }which can also be set per-entrytype and per-entry. The two modes available are <entry> mode and <db> mode. The default mode is <entry> mode.

In <entry> multiscript mode, the variant specifications for a field exist to inform only \emph{how} a field is printed. In <db> multiscript mode, the variant specifications are used to specify both \emph{which} variant is printed \emph{and} how it is printed.

In <entry> mode, \biblatex uses the variant information in the field name to potentially switch language settings, depending on the \opt{autolangshort*} options. It is guaranteed that in <entry> mode, there is no more than one variant per field in an entry as \biber will ensure this---any duplicate fields, regardless of the variant, will be overwritten. This mode is intended for automatic language switching for entries where the language/script of fields may not all be the same, as in a French article in an Italian collection. For example, given the following entry in a document typeset with babel/polyglossia main language set to <french> and with \opt{autolangshort} set to <hyphenation> (see the \biber manual for details about this extended variant \file{.bib} format):
...
@incollection{Canart1982,
  ...
  SERIES_italian       = {Pubblicazioni della Università Cattolica
                          del Sacro Cuore. Scienze filologiche e
                          letteratura},
  TITLE                = {Cinq manuscrits transférés directement
                          du monastère de Stoudios à celui de Grottiest},
  YEAR                 = {1982}
}
\end{lstlisting}

\biblatex would print the \bibfield{BOOKSUBTITLE}, \bibfield{BOOKTITLE}, \bibfield{PUBLISHER} and \bibfield{SERIES} fields using italian hyphenation. The \bibfield{PUBLISHER} with no variant specification above would be overwritten by the succeeding one by \biber, thus guaranteeing that only one variant of each field is present in the \file{.bbl} in <entry> mode. In <entry> mode, the data printing commands:
...
\end{lstlisting}
%
do not need to be qualified with the variants in the \file{.bib} file since there should be no more than one variant per field and \biblatex with automatically use the field variant to print the field.

<db> mode is intended for entries where multiple variants of fields are maintained in the same entry, as for a book in Cyrillic script which sometimes needs to be cited in romanised form or even in translated form and language. The particular form/lang selected to be printed are determined by the \opt{vform}, \opt{vlang} and \opt{vtranslang} options. Here is an example---suppose the \bibtex format bibliography contained the following:
...
\end{lstlisting}
%
With options \kvopt{vform}{original}, \kvopt{vlang}{russian} and \kvopt{vtranslang}{english}, \biber would output a \file{.bbl} entry like this:
...
\end{lstlisting}
%
In this example, when \biblatex reads the \file{.bbl}, a few things are to be noted:

\begin{itemize}
\setlength{\itemsep}{0pt}
\item The \bibfield{labelname} is implicitly \kvopt{form}{original} and \kvopt{lang}{russian}
\item The \kvopt{form}{translated} fields are all \kvopt{lang}{english} due to the global \opt{vtranslang} setting.
\end{itemize}

The fields in the \file{.bbl} can then all be accessed by the usual \biblatex macros which are all variant aware so that one is able, for example, to select a particular variant of a field in a style:
\begin{lstlisting}[style=latex]
\printname[][][form=uniform]{author}
\printlist[][][form=romanised]{publisher}
\printfield[][form=translated,lang=german]{title}
\end{lstlisting}
%
Macros which allow specification of variants take an optional \prm{variantspec} parameter which is a keyval item consisting of a \kvopt{form}{form} option and/or a \kvopt{lang}{lang} option.

All macros which allow specific variants to be selected default the \opt{form} option to the \opt{vform} option value, the \opt{lang} option to the \opt{vlang} option value and for \opt{translated} forms, the \opt{lang} option to the \opt{vtranslang} option value. Hence, in the above example, specifying \kvopt{lang}{english} for the translated \bibfield{title} field is not strictly necessary.

If a data source entry contains a variant enabled field in both an unqualified and equivalent qualified variant (given the \opt{vform} and \opt{vlang} option settings), then the last one in the entry is used. For example, given \kvopt{vform}{original} and \kvopt{vlang}{english}:
...
\end{lstlisting}
%
would result in the \bibfield{author} being set to «Jack Jones».

There are a few helper macros which reset \opt{vform}, \opt{vlang} and \opt{vtranslang} globally within \biblatex (but don't change what is passed to \biber which always gets the global package settings or defaults for these options).

\begin{ltxsyntax}
\cmditem{vform}{form}\BiberOnlyMark
\cmditem{vlang}{lang}\BiberOnlyMark
\cmditem{vtranslang}{lang}\BiberOnlyMark

These macros redefine globally the default \opt{vform}, \opt{vlang} or \opt{vtranslang} options. This is useful since the \cmd{print*} macros use these defaults in case no explicit option is passed and so you could print two bibliographies, one for the \opt{original} field variant forms and one for the \opt{romanised} forms like this:
\end{ltxsyntax}
...
\end{lstlisting}
%
You might want to switch these global settings but only in order to pick up fields which have the specified forms/langs. This is the purpose of the \cmd{vfallback*} commands:

\begin{ltxsyntax}
\cmditem{vfallbackon}\BiberOnlyMark
\cmditem{vfallbackoff}\BiberOnlyMark
\end{ltxsyntax}
%
Use \cmd{vfallbackon} to turn variant fallback on and \cmd{vfallbackoff} to turn it off. The default is \cmd{vfallbackon}. Variant fallback means that if the current global variant is not found for a field, a fallback will be looked for using the global default form and lang (specified via the package options). Such a fallback ignores any currently modified global values set by using \cmd{vform}, \cmd{vlang} or \cmd{vtranslang}. Not that variant fallback does not apply when you explicitly specify a variant using the \opt{form/lang} arguments to a macro which allows variant specification; it is only used in cases where you specify no variant explicitly and rely on the current global settings. For example, perhaps you have an entry like this:

\begin{lstlisting}[style=bibtex]
author = {Alan Smith},
title = {Un Titel},
title_translated_english = {A title}
\end{lstlisting}
%
You could print two bibliographies, one with french titles and one with english. Assuming that your package \opt{vlang} option is set to <\opt{french}>:
...
\end{lstlisting}
%
Since \cmd{vfallbackon} is enabled by default, this means that even though the \bibfield{author} has no \opt{translated/english} variant (as it is by default \opt{original/french} due to the package options), it will be used anyway as a fallback. With \cmd{vfallbackoff}, the entry in the second bibliography would be missing the \bibfield{author} field since there is no \opt{translated/english} author field. Variant fallback is only used when \opt{msmode} is set to <db> since it makes no sense in <entry> multiscript mode.

\begin{ltxsyntax}
\cmditem{DeclareVariantFallbacks}{specification}\BiberOnlyMark

The \prm{specification} consists of one or more \cmd{vfallback} commands:

\cmditem{vfallback}

Each \cmd{vfallback} macro takes following optional arguments (although at least one must be specified):

\begin{optionlist*}
\choitem{form}{\prm{form}}
\choitem{lang}{\prm{lang}}
\end{optionlist*}
\end{ltxsyntax}
%
\cmd{DeclareVariantFallbacks} defines a fallback chain of variants which \biber uses  if no value is found when looking for a variant corresponding to the global defaults (defined with \cmd{vform}, \cmd{vlang}/\cmd{vtranslang}). The default value is:
...
\end{lstlisting}
%
This means that if when sorting, define labels, processing inheritance etc. and \biber is looking for, say, a translated variant of a field by default due to the global \cmd{vform} option, if there is no such variant for a field, \biber will look first for an <original> form of the field and then for a <romanised> form before giving up. \biber also uses this fallback chain when filling in null list entries as described in \secref{aut:bib:dat}.
...
The fields listed in this section are the regular ones holding printable data in the default data model. The name on the left is the default data model name of the field as used by \sty{biblatex} and its backend. The \sty{biblatex} data type is given to the right of the name. See \secref{bib:fld:typ} for explanation of the various data types. The variant enabled fields in the default data model are marked in the right margin.
...
\fielditem{abstract}{literal\VEMark}
...
\fielditem{addendum}{literal\VEMark}
...
\listitem{afterword}{name\VEMark}
...
\fielditem{annotation}{literal\VEMark}
...
\listitem{annotator}{name\VEMark}
...
\listitem{bookauthor}{name\VEMark}
...
\fielditem{booksubtitle}{literal\VEMark}
...
\fielditem{booktitle}{literal\VEMark}
...
\fielditem{edition}{integer or literal\VEMark}
...
\fielditem{eventtitle}{literal\VEMark}
...
\fielditem{eventtitleaddon}{literal\VEMark}
...
\fielditem{howpublished}{literal\VEMark}
...
A title to use for indexing instead of the regular \bibfield{title} field. This field may be useful if you have an entry with a title like «An Introduction to \dots» and want that indexed as «Introduction to \dots, An». Style authors should note that when using \bibtex as the backend, \sty{biblatex} automatically copies the value of the \bibfield{title} field to \bibfield{indextitle} if the latter field is undefined. When using \biber as the backend, \sty{biblatex} automatically copies the value of the variant of the \bibfield{title} field specified by the \opt{vform} form and the \opt{vlang} language (or \opt{vtranslang} language if \kvopt{form}{translated}) to \bibfield{indextitle} if the latter field is undefined.

\listitem{institution}{literal\VEMark}
...
\listitem{introduction}{name\VEMark}
...
\fielditem{issue}{literal\VEMark}
...
\fielditem{issuesubtitle}{literal\VEMark}

The subtitle of a specific issue of a journal or other periodical.

\fielditem{issuetitle}{literal\VEMark}
...
\fielditem{journalsubtitle}{literal\VEMark}

The subtitle of a journal, a newspaper, or some other periodical.

\fielditem{journaltitle}{literal\VEMark}
...
\fielditem{library}{literal\VEMark}
...
\listitem{location}{literal\VEMark}
...
\fielditem{maintitleaddon}{literal\VEMark}
...
\fielditem{nameaddon}{literal\VEMark}
...
\listitem{organization}{literal\VEMark}
...
\listitem{origlocation}{literal\VEMark}
...
\listitem{origpublisher}{literal\VEMark}
...
\fielditem{origtitle}{literal\VEMark}
...
\listitem{publisher}{literal\VEMark}
...
\fielditem{shorthandintro}{literal\VEMark}
...
\fielditem{subtitle}{literal\VEMark}

The subtitle of the work.

\fielditem{title}{literal\VEMark}

The title of the work.

\fielditem{titleaddon}{literal\VEMark}

An annex to the \bibfield{title}, to be printed in a different font.

\listitem{translator}{name\VEMark}
...
\fielditem{venue}{literal\VEMark}
...
Sets all of the date options (\opt{date}, \opt{origdate}, \opt{eventdate}, \opt{urldate}) to the same value.
...
\optitem[none]{autolangshort}{\opt{none}, \opt{hyphen}, \opt{other}, \opt{other*}}[\BiberOnly]

As \opt{autolang} but controls the language environment for short text printed via per-field automatic language switching for variant enabled fields with a language specification. See \secref{bib:fld:var}.

\boolitem[false]{autolangshortstrings}[\BiberOnly]

When \opt{autolangshort} is set to anything other than \opt{none}, this option determines whether to also switch defined bibliography localization strings to the target language. See \secref{aut:lng:key}.
...
\optitem[entry]{msmode}{\opt{db}, \opt{entry}}[\BiberOnly]

Selects the global multiscript mode. See \secref{bib:fld:var}.
...
The sorting order of shorthands with \biber is more flexible and is set with the \opt{sorting} option to the \cmd{printshorthands} command.
...
\optitem[original]{vform}{\prm{form}}[\BiberOnly]

In macros which support selecting a variant of a data field, the default form to be selected when no form is specified. Can also be set per-entry. See \secref{bib:fld:var}. For allowed variant forms, see \secref{bib:fld:var}.

\optitem{vlang}{\prm{language}}[\BiberOnly]

In macros which support selecting a variant of a data field, the default language to be selected when no language is specified. If not specified, defaults to the \sty{babel}/\sty{polyglossia} main language or otherwise the language specified by the \opt{language} option. Can also be set per-entry. \biber automatically copies any \bibfield{langid} field in a data source entry into this option on a per-entry basis in order to aid multilingual bibliography processing. See \secref{bib:fld:var}.

\optitem{vtranslang}{\prm{language}}[\BiberOnly]

In macros which support selecting a variant of a data field, the default language to be selected when no language is specified and the «translated» form is selected. If not specified, defaults to the \sty{babel}/\sty{polyglossia} main language or otherwise the language specified by the \opt{language} option. Can also be set per-entry. See \secref{bib:fld:var}. 
...
\choitem[original]{labelnameform}{\prm{form}}

Specifies the form for the variant enabled field specified with \opt{labelnamefield}, if the datasource you are using supports this. For allowed variant forms, see \secref{bib:fld:var}.

\choitem[none]{labelnamelang}{\prm{language specifier}}

Specifies the \sty{babel}/\sty{polyglossia} language for the variant enabled field specified with \opt{labelnamefield} if the datasource you are using supports this. The default, if no \prm{lang} option is set, this means to use the field variant in the datasource which has no explicit language defined.
...
\choitem[original]{labeltitleform}{\prm{form}}

Specifies the form for the variant enabled field specified with \opt{labeltitlefield}, if the datasource you are using supports this. For allowed variant forms, see \secref{bib:fld:var}.

\choitem[none]{labeltitlelang}{\prm{language specifier}}

Specifies the \sty{babel}/\sty{polyglossia} language for the variant enabled field specified with \opt{labeltitlefield} if the datasource you are using supports this. The default, if no \prm{lang} option is set, means to use the field variant in the datasource which has no explicit language defined.
...
These commands print the authors. Strictly speaking, it prints the \bibfield{labelname} list, which may be the \bibfield{author}, the \bibfield{editor}, or the \bibfield{translator} (\bibtex) or which can be defined using the \cmd{DeclareLabelname} macro (\biber, see \secref{aut:ctm:fld}). \cmd{Citeauthor} is similar to \cmd{citeauthor} but capitalizes the name prefix of the first name in the citation if the \opt{useprefix} option is enabled, provided that there is a name prefix. The starred forms effectively force maxcitenames to 1 for just this command on so only print the first name in the \bibfield{labelname} list (potentially followed by the «et al» string if there are more names). This allows more natural textual flow when refering to a paper in the singular when otherwise \cmd{citeauthor} would generate a (naturally plural) list of names.

\cmditem{citetitle}[prenote][postnote]{key}
\cmditem*{citetitle*}[prenote][postnote]{key}

This command prints either the \bibfield{labeltitle} field (\bibtex) or the title field selected with \cmd{DeclareLabeltitle} (\biber, see \secref{aut:ctm:fld}). It will use the abridged title in the \bibfield{shorttitle} field, if available. The starred form always prints the full title found in the \bibfield{title} field.

\cmditem{citeyear}[prenote][postnote]{key}
\cmditem*{citeyear*}[prenote][postnote]{key}

This command prints the year (\bibfield{year} field or year component of \bibfield{date}). The starred form includes the \bibfield{extrayear} information, if any.

\cmditem{citedate}[prenote][postnote]{key}
\cmditem*{citedate*}[prenote][postnote]{key}

This command prints the full date (\bibfield{date} or \bibfield{year}). The starred form includes the \bibfield{extrayear} information, if any.
...
\BiberOnlyMark When selecting a particular language variant of a field or list using the \prm{variantspec} option (see \secref{bib:fld:var}), the \opt{autolangshort} package option determines which Babel/Polyglossia language environment to use when printing the field/list. The \opt{autolangshortstrings} package option determines whether localization string switching will be performed in addition to Babel/Polyglossia language environment switching. See the example in the documentation for \cmd{printlist} in \secref{aut:bib:dat}.

\begin{ltxsyntax}

\cmditem{citename}[prenote][postnote]{key}[format][variantspec]{name list}
...
\cmditem{citelist}[prenote][postnote]{key}[format][variantspec]{literal list}
...
\cmditem{citefield}[prenote][postnote]{key}[format][variantspec]{field}
...
\fielditem{labelnamesourcefield}{literal\BiberOnlyMark}

This field holds the name of the field used to generate the \bibfield{labelname} field.

\fielditem{labelnamesourceform}{literal\BiberOnlyMark}

This field holds the form of the variant of the field used to generate the \bibfield{labelname} field.

\fielditem{labelnamesourcelang}{literal\BiberOnlyMark}

This field holds the language of the variant of the field used to generate the \bibfield{labelname} field.

\fielditem{labeltitlesourcefield}{literal\BiberOnlyMark}

This field holds the name of the field used to generate the \bibfield{labeltitle} field.

\fielditem{labeltitlesourceform}{literal\BiberOnlyMark}

This field holds the form of the variant of the field used to generate the \bibfield{labeltitle} field.

\fielditem{labeltitlesourcelang}{literal\BiberOnlyMark}

This field holds the language of the variant of the field used to generate the \bibfield{labeltitle} field.
...
\BiberOnlyMark Commands which support variant enabled fields allow the specification of an optional \prm{variantspec} option (see \secref{bib:fld:var}) in which case they operate on only the specified field variant. When selecting a particular language variant of the field using the \prm{variantspec} option, the \opt{autolangshort} package option determines which Babel/Polyglossia language environment to use when printing the field/list. The \opt{autolangshortstrings} package option determines whether localization string switching will be performed in addition to Babel/Polyglossia language environment switching. See the example in \cmd{printlist} below.

\begin{ltxsyntax}

\cmditem{printfield}[format][variantspec]{field}
...
\cmditem{printlist}[format][start\ensuremath\rangle--\ensuremath\langle stop][variantspec]{literal list}
...
Here is an example involving a variant enabled \bibfield{location} list. Assuming \opt{autolangshort=other*} and \opt{autolangshortstrings=true} along with the following \file{.bib} field:

\begin{lstlisting}[style=bibtex]
location_translated_german = {place1a"=place1b and place2}
\end{lstlisting}
%
Then the following macro call:

\begin{lstlisting}[style=latex]
\printlist[][][form=translated,lang=german]{location}
\end{lstlisting}
%
would result in «place1a-place1b und place2». Note that the the \opt{autolangshort} setting has caused the Babel/Polyglossia shorthand to be interpreted (assuming you loaded Babel/Polyglossia with shorthands enabled) and also that the \opt{autolangshortstrings} setting caused the \biblatex \file{german.lbx} to be loaded and the «\opt{and}» localization string to be switched.

\cmditem{printlistelement}[format]{element}[variantspec]{name list}

This command allows the printing of a specific element within a plain list as specified by the \prm{element} argument which should be a valid plain list counter value. \cmd{printlistelement} does not check for valid counter values for the \prm{element} argument. The \prm{format} must be defined with \cmd{DeclareListElementFormat}. This command is specifically intended for use within name formatting directives defined with \cmd{DeclareListFormat}. It allows the user to refer to a different variant of the same list so that lists such as the following can be formatted:\\[2ex]

Москва (Moscow)\\[2ex]

Given the \file{.bib} entry fragment:
...
\begin{lstlisting}[style=latex]
\DeclareListElementFormat{test}{%
  \mkbibparens{\mkbibemph{#1}}
\DeclareListFormat{test}{%
  #1\space\printlistelement[test]{\value{listcount}}[form=translated]{location}
}
\printlist[test]{location}
\end{lstlisting}
%
The formats in the example are very basic so that the idea is clear. In general, you will pass |\value{listcount}| as the \prm{element} so that you refer to the same particular list element in the other variant. Note that the formats have the same name for both \cmd{printlist} and \cmd{printlistelement}---this is not necessary but just to indicate that internally, the formats are in different namespaces so that you won't get infinite loops.

\cmditem{printnames}[format][start\ensuremath\rangle--\ensuremath\langle stop][variantspec]{name list}
...
\cmditem{printnameelement}[format]{element}[variantspec]{name list}

This command allows the printing of a specific name within a name list as specified by the \prm{element} argument which should be a valid name list counter value. \cmd{printnameelement} does not check for valid counter values for the \prm{element} argument. The \prm{format} must be defined with \cmd{DeclareNameElementFormat}. This command is specifically intended for use within name formatting directives defined with \cmd{DeclareNameFormat}. It allows the user to refer to a different variant of the same name so that name lists such as the following can be formatted:\\[2ex]

Булгаков, Павел (Bulgakov, Pavel) and Розенфельд, Борис (Rosenfeld, Boris)\\[2ex]
...
\DeclareNameFormat{test}{%
  #1, #3\space\printnameelement[test]{\value{listcount}}[form=uniform]{author}
}
\printnames[test]{author}
\end{lstlisting}
%
The formats in the example are very basic so that the idea is clear. In general, you will pass |\value{listcount}| as the \prm{element} so that you refer to the same particular name in the other variant. Note that the formats have the same name for both \cmd{printnames} and \cmd{printnameelement}---this is not necessary but just to indicate that internally, the formats are in different namespaces so that you won't get infinite loops.

It is also possible to omit a particular list element variant in case it is not needed:

\begin{lstlisting}[style=bibtex,escapechar=+]
  AUTHOR = {+Булгаков, Павел and Розенфельд, Борис and John Smith+},
  AUTHOR_uniform = {Bulgakov, Pavel and Rosenfeld, Boris and *},
\end{lstlisting}
%
Here the |*| marks a null list element. The particular character which marks this in a name list should correspond to the \biber option |--variant-null-list| (the default is |*|). \biber will resolve such null list elements using the variant fallback chain specified with \cmd{DeclareVariantFallbacks} (see \secref{bib:fld:var}).
...
\cmditem{indexfield}[format][variantspec]{field}
...
\cmditem{indexlist}[format][start\ensuremath\rangle--\ensuremath\langle stop][variantspec]{literal list}
...
\cmditem{indexnames}[format][start\ensuremath\rangle--\ensuremath\langle stop][variantspec]{name list}
...
\cmditem{DeclareListElementAlias}[entry type]{alias}[format entry type]{format}

Declares \prm{alias} to be an alias for the list element format \prm{format}. If an \prm{entrytype} is specified, the alias is specific to that type. The \prm{format entry type} is the entry type of the backend format. This is only required when declaring an alias for a type"=specific formatting directive.
...
\cmditem{DeclareNameElementAlias}[entry type]{alias}[format entry type]{format}

Declares \prm{alias} to be an alias for the name element format \prm{format}. If an \prm{entrytype} is specified, the alias is specific to that type. The \prm{format entry type} is the entry type of the backend format. This is only required when declaring an alias for a type"=specific formatting directive.
...
\valitem{fieldsourceform}{form}
\valitem{fieldsourcelang}{language}
\valitem{fieldtarget}{entryfield}
\valitem{fieldtargetform}{form}
\valitem{fieldtargetlang}{language}
\valitem{match}{regexp}
\valitem{notmatch}{regexp}
\valitem{replace}{regexp}
\valitem{fieldset}{entryfield}
\valitem{fieldsetform}{form}
\valitem{fieldsetlang}{language}
\valitem{fieldvalue}{string}
\boolitem[false]{append}
\boolitem[false]{final}
\boolitem[false]{nogeneralisevariants}
\boolitem[false]{null}
...
\item Change the \texttt{typesource} \prm{entrytype} to the
  \texttt{typetarget} \prm{entrytype}, if defined. If \texttt{final}
  is \texttt{true} then if the \prm{entrytype} of the entry is not
  \texttt{typesource}, processing of the parent \cmd{map} immediately
  terminates.
\item Change the \texttt{fieldsource} \prm{entryfield} (optionally a
  particular variant specified with \texttt{fieldsourceform} and/or
  \texttt{fieldsourcelang}) to \texttt{fieldtarget} (optionally a
  particular variant specified with \texttt{fieldtargetform} and/or
  \texttt{fieldtargetlang}), if defined. If neither
  \texttt{fieldsource} nor \texttt{fieldlang} are specified, all
  variants of \texttt{fieldsource} will be mapped to corresponding
  variants of \texttt{fieldtarget} (unless
  \texttt{nogeneralisevariants} is defined for this step). If only
  \texttt{fieldsourceform} is specified, the mapping will select the
  variant of the field with the specified form, whatever the language
  of the field variant. If there is more than one field variant with
  the same \texttt{fieldsourceform} and different language, the
  \texttt{fieldsource} will not be selected. If only
  \texttt{fieldsourcelang} is specified, the mapping will select the
  variant of the field with the specified language, whatever the form
  of the field variant. If there is more than one field variant with
  the same \texttt{fieldsourcelang} and different form, the
  \texttt{fieldsource} will not be selected. If \texttt{final} is
  \texttt{true} then if there is no \texttt{fieldsource}
  \prm{entryfield} in the entry, processing of the parent \cmd{map}
  immediately terminates.
\item If \texttt{match} is defined but \texttt{replace} is not, only
  apply the step if the \texttt{fieldsource} \prm{entryfield} matches
  the \texttt{match} regular expression (logic is reversed if you use
  \texttt{notmatch} instead)\footnote{Regular expressions are full
    Perl 5.16 regular expressions. This means you may need to deal
    with special characters, see examples.}. You may use capture
  parenthesis as usual and refer to these (\$1\ldots\$9) in later
  \texttt{fieldvalue} specifications. This allows you to pull out
  parts of some fields and put these parts in other fields.
\item Perform a regular expression match and replace on the value of
  the \texttt{fieldsource} \prm{entryfield} if \texttt{match} and
  \texttt{replace} are defined. If \texttt{fieldsourceform} and
  \texttt{fieldsourcelang} are not specified, operates on all variants
  of \texttt{fieldsource} unless \texttt{nogeneralisevariants} is
  defined for this step.
\item If \texttt{fieldset} is defined, then its value is
  \prm{entryfield} which will be set to a value specified by further
  options (optionally a specific variant chosen using
  \texttt{fieldsetform} and \texttt{fieldsetlang}). If neither are
  specified, all variants of \texttt{fieldset} will be operated on
  when specifying \texttt{null} or \texttt{fieldvalue} (unless
  \texttt{nogeneralisevariants} is defined for this step). If
  \texttt{overwrite} is false for this step and the field to set
  already exists then the map step is ignored. If \texttt{final} is
  also true for this step, then processing of the parent map stops at
  this point. If \texttt{append} is true, then the value to set is
  appended to the current value of \prm{entryfield}. The value to set
  is specified by a mandatory one and only one of the following
  options:
  \begin{itemize}
...
\DeclareSourcemap{
  \maps[datatype=bibtex]{
    ...
    \map{
      \step[fieldset=<<note>>, null, nogeneralisevariants]
    }
  }
}
\end{lstlisting}
%
Removes all variants of the \bibfield{addendum} field. Removes only the variant unqualified \bibfield{note} field, leaving any specific variants in the entry.
...
\choitem[original]{form}{\prm{form}}

Specifies the form for the variant enabled field specified with \opt{field}, if the datasource you are using supports this. If absent, defaults to the value of the \opt{vform} option. See \secref{bib:fld:var} and \secref{use:opt:pre:gen}. For allowed variant forms, see \secref{bib:fld:var}.

\choitem[none]{lang}{\prm{language}}

Specifies the \sty{babel}/\sty{polyglossia} language for the variant enabled field specified with \opt{field} if the datasource you are using supports this. If absent, defaults to the value of the \opt{vlang} option or \opt{vtranslang} if the \kvopt{form}{translated}. See \secref{bib:fld:var} and \secref{use:opt:pre:gen}.
...
\end{lstlisting}
%
Variants of names can contain a null name marker (defined with the \biber option |--variant-null-list|) which indicate that the variant for a particular name list element is the same as the corresponding name list element in the default name variant. When building labels from name lists which contain such markers, the default variant name element will be used. Consider:
...
\end{lstlisting}
%
With a simple label consisting of three characters from the left side of each name of the |uniform| variant, the label would be '|BulRosSmi|'.
...
The \cmd{field} element adds a \prm{field} to the sorting specification. If the \prm{field} is undefined in the entry being sorted, the element is skipped. The \cmd{field} command supports the following \prm{options}:
...
\end{optionlist*}
%
For example, the following specification for selecting \bibfield{labelname} first looks for an english translation variant of the \bibfield{AUTHOR} field, then a romanised variant and if neither of these can be found, the default field with no variant specification.
\begin{lstlisting}[style=latex]
\DeclareLabelname{%
  \field[form=translated,lang=english]{author}
  \field[form=romanised]{author}
...
\end{optionlist*}
%
For example, the following specification for selecting \bibfield{labeltitle} first looks for an English translation variant of the \bibfield{TITLE} field, then a romanised variant and if neither of these can be found, the default field with no variant specification.
\begin{lstlisting}[style=latex]
\DeclareLabeltitle{%
...
\cmditem{inherit}[options]{source}{target}

Defines an inheritance rule by mapping a \prm{source} field to a \prm{target} field. The following \prm{option}s may be specified:

\begin{optionlist*}

\boolitem[false]{override} Per-field value of the option mentioned above. If set here, it takes precedence over any global options set with \cmd{DefaultInheritance}.

\choitem[original]{sourceform}{\prm{form}}

Specifies the form for the variant enabled field specified with \opt{source}, if the datasource you are using supports this. For allowed variant forms, see \secref{bib:fld:var}.

\choitem[none]{sourcelang}{\prm{language specifier}}

Specifies the \sty{babel}/\sty{polyglossia} language for the variant enabled field specified with \opt{source} if the datasource you are using supports this. The default, if no \prm{lang} option is set, means to use the field variant in the datasource which has no explicit language defined.

\choitem[original]{targetform}{\prm{form}}

Specifies the form for the variant enabled field specified with \opt{target}, if the datasource you are using supports this. For allowed variant forms, see \secref{bib:fld:var}.

\choitem[none]{targetlang}{\prm{language specifier}}

Specifies the \sty{babel}/\sty{polyglossia} language for the variant enabled field specified with \opt{target} if the datasource you are using supports this. The default, if no \prm{lang} option is set, means to use the field variant in the datasource which has no explicit language defined.
...
\begin{lstlisting}[style=latex]
\DeclareDataInheritance{<<collection>>}{<<incollection>>}{
  \inherit[sourceform=romanised, targetform=romanised]{<<title>>}{<<booktitle>>}
  \inherit[sourceform=romanised, targetform=romanised]{<<subtitle>>}{<<booksubtitle>>}
  \inherit[sourceform=romanised, targetform=romanised]{<<titleaddon>>}{<<booktitleaddon>>}
}
\end{lstlisting}
%
This example is the same as the previous one except that it demonstrates inheritance of the romanised variants of the source fields as romanised variants of the target fields.
...
\cmditem{thefield}[variantspec]{field}

Expands to the unformatted \prm{field}. If the \prm{field} is undefined, this command expands to an empty string. If \prm{variantspec} is specified, expands to a particular field variant (see \secref{bib:fld:var}).
...
\cmditem{usefield}{command}[variantspec]{field}

Executes \prm{command} using the unformatted \prm{field} as its argument. If \prm{variantspec} is specified, uses a particular field variant (see \secref{bib:fld:var}).

\cmditem{thelist}[variantspec]{literal list}

Expands to the unformatted \prm{literal list}. If the list is undefined, this command expands to an empty string. Note that this command will dump the \prm{literal list} in the internal format used by this package. This format is not suitable for printing. If \prm{variantspec} is specified, expands to a particular list variant (see \secref{bib:fld:var}).

\cmditem{thename}[variantspec]{name list}

Expands to the unformatted \prm{name list}. If the list is undefined, this command expands to an empty string. Note that this command will dump the \prm{name list} in the internal format used by this package. This format is not suitable for printing. If \prm{variantspec} is specified, expands to a particular name list variant (see \secref{bib:fld:var}).

\cmditem{savefield}[variantspec]{field}{macro}
\cmditem*{savefield*}[variantspec]{field}{macro}

Copies an unformatted \prm{field} to a \prm{macro}. The regular form of this command defines the \prm{macro} globally, the starred one works locally.\\
\BiberOnlyMark For variant enabled fields, saves all field variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a field which is not variant enabled.

\cmditem{savelist}[variantspec]{literal list}{macro}
\cmditem*{savelist*}[variantspec]{literal list}{macro}

Copies an unformatted \prm{literal list} to a \prm{macro}. The regular form of this command defines the \prm{macro} globally, the starred one works locally.\\
\BiberOnlyMark For variant enabled lists, saves all list variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a list which is not variant enabled.

\cmditem{savename}[variantspec]{name list}{macro}
\cmditem*{savename*}[variantspec]{name list}{macro}

Copies an unformatted \prm{name list} to a \prm{macro}. The regular form of this command defines the \prm{macro} globally, the starred one works locally.\\
\BiberOnlyMark For variant enabled name lists, saves all name list variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a name list which is not variant enabled.

\cmditem{savefieldcs}[variantspec]{field}{csname}
\cmditem*{savefieldcs*}[variantspec]{field}{csname}
...
\cmditem{savelistcs}[variantspec]{literal list}{csname}
\cmditem*{savelistcs*}[variantspec]{literal list}{csname}
...
\cmditem{savenamecs}[variantspec]{name list}{csname}
\cmditem*{savenamecs*}[variantspec]{name list}{csname}
...
\cmditem{restorefield}[variantspec]{field}{macro}

Restores a \prm{field} from a \prm{macro} defined with \cmd{savefield} before. The field is restored within a local scope.\\
\BiberOnlyMark For variant enabled fields, restores all field variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a field which is not variant enabled.

\cmditem{restorelist}[variantspec]{literal list}{macro}

Restores a \prm{literal list} from a \prm{macro} defined with \cmd{savelist} before. The list is restored within a local scope.\\
\BiberOnlyMark For variant enabled lists, restores all list variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a list which is not variant enabled.

\cmditem{restorename}[variantspec]{name list}{macro}

Restores a \prm{name list} from a \prm{macro} defined with \cmd{savename} before. The list is restored within a local scope.\\
\BiberOnlyMark For variant enabled name lists, restores all name list variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a name list which is not variant enabled.

\cmditem{clearfield}[variantspec]{field}

Clears the \prm{field} within a local scope. A field cleared in this way is treated as undefined by subsequent data commands.\\
\BiberOnlyMark For variant enabled fields, clears all field variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a field which is not variant enabled.

\cmditem{clearlist}[variantspec]{literal list}

Clears the \prm{literal list} within a local scope. A list cleared in this way is treated as undefined by subsequent data commands.\\
\BiberOnlyMark For variant enabled lists, clears all list variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a list which is not variant enabled.

\cmditem{clearname}[variantspec]{name list}

Clears the \prm{name list} within a local scope. A name list cleared in this way is treated as undefined by subsequent data commands.\\
\BiberOnlyMark For variant enabled name lists, clears all name list variants unless a specific variant is specified with the \prm{variantspec} option (see \secref{bib:fld:var}). You will receive a warning if you specify a \prm{variantspec} for a name list which is not variant enabled.
...
\cmditem{iffieldundef}[variantspec]{field}{true}{false}

Expands to \prm{true} if the \prm{field} is undefined, and to \prm{false} otherwise. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).

\cmditem{iflistundef}[variantspec]{literal list}{true}{false}

Expands to \prm{true} if the \prm{literal list} is undefined, and to \prm{false} otherwise. If \prm{variantspec} is specified, applies to a particular list variant (see \secref{bib:fld:var}).

\cmditem{ifnameundef}[variantspec]{name list}{true}{false}

Expands to \prm{true} if the \prm{name list} is undefined, and to \prm{false} otherwise. If \prm{variantspec} is specified, applies to a particular name list variant (see \secref{bib:fld:var}).

\cmditem{iffieldsequal}[variantspec]{field 1}[variantspec]{field 2}{true}{false}

Expands to \prm{true} if the values of \prm{field 1} and \prm{field 2} are equal, and to \prm{false} otherwise.\\
\BiberOnlyMark When \prm{field 1} or \prm{field 2} is a variant enabled field, a warning will be generated if a comparison is attempted between several variants and a single variant or between several variants and non variant enabled field. Comparing two variant enabled fields with no \prm{variantspec} option (see \secref{bib:fld:var}) specified for either field will compare \emph{all} variants of the fields and return true if and only if the two fields have exactly the same variants and the values of those variants are identical. For example, suppose \prm{vfield 1} and \prm{vfield 2} are both variant enabled fields and \prm{nvfield3} is a non variant enabled field:

\begin{lstlisting}[style=latex]
\iffieldsequal[form=translated,lang=german]{<<vfield1>>}{<<vfield2>>}{true}{false}
\end{lstlisting}
%
Will generate a warning as we are trying to compare one particular variant of \prm{vfield1} with all variants of \prm{vfield2}.

\begin{lstlisting}[style=latex]
\iffieldsequal[form=translated,lang=german]{<<vfield1>>}{<<nvfield3>>}{true}{false}
\end{lstlisting}
%
This example is fine as we are comparing one particular variant of \prm{vfield1} with a non variant enabled field \prm{nvfield3}.
%
\begin{lstlisting}[style=latex]
\iffieldsequal[form=translated,lang=german]{<<vfield1>>}
              [form=translated,lang=german]{<<vfield2>>}{true}{false}
\end{lstlisting}
%
This is also fine as we are comparing specific variants of two variant enabled fields

\cmditem{iflistsequal}[variantspec]{literal list 1}[variantspec]{literal list 2}{true}{false}

Expands to \prm{true} if the values of \prm{literal list 1} and \prm{literal list 2} are equal, and to \prm{false} otherwise. Comparison of variant enabled lists is handled as for \cmd{iffieldsequal} above.

\cmditem{ifnamesequal}[variantspec]{name list 1}[variantspec]{name list 2}{true}{false}

Expands to \prm{true} if the values of \prm{name list 1} and \prm{name list 2} are equal, and to \prm{false} otherwise. Comparison of variant enabled name lists is handled as for \cmd{iffieldsequal} above.

\cmditem{iffieldequals}[variantspec]{field}{macro}{true}{false}

Expands to \prm{true} if the value of the \prm{field} is equal to the definition of \prm{macro}, and to \prm{false} otherwise. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).

\cmditem{iflistequals}[variantspec]{literal list}{macro}{true}{false}

Expands to \prm{true} if the value of the \prm{literal list} is equal to the definition of \prm{macro}, and to \prm{false} otherwise. If \prm{variantspec} is specified, applies to a particular list variant (see \secref{bib:fld:var}).

\cmditem{ifnameequals}[variantspec]{name list}{macro}{true}{false}

Expands to \prm{true} if the value of the \prm{name list} is equal to the definition of \prm{macro}, and to \prm{false} otherwise. If \prm{variantspec} is specified, applies to a particular name list variant (see \secref{bib:fld:var}).

\cmditem{iffieldequalcs}[variantspec]{field}{csname}{true}{false}
...
\cmditem{iflistequalcs}[variantspec]{literal list}{csname}{true}{false}
...
\cmditem{ifnameequalcs}[variantspec]{name list}{csname}{true}{false}
...
\cmditem{iffieldequalstr}[variantspec]{field}{string}{true}{false}

Executes \prm{true} if the value of the \prm{field} is equal to \prm{string}, and \prm{false} otherwise. This command is robust. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).

\cmditem{iffieldxref}[variantspec]{field}{true}{false}

If the \bibfield{crossref}\slash \bibfield{xref} field of an entry is defined, this command checks if the \prm{field} is related to the cross"=referenced parent entry. It executes \prm{true} if the \prm{field} of the child entry is equal to the corresponding \prm{field} of the parent entry, and \prm{false} otherwise. If the \bibfield{crossref}\slash \bibfield{xref} field is undefined, it always executes \prm{false}. This command is robust. See the description of the \bibfield{crossref} and \bibfield{xref} fields in \secref{bib:fld:spc} as well as \secref{bib:cav:ref} for further information concerning cross"=referencing. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).

\cmditem{iflistxref}[variantspec]{literal list}{true}{false}

Similar to \cmd{iffieldxref} but checks if a \prm{literal list} is related to the cross"=referenced parent entry. See the description of the \bibfield{crossref} and \bibfield{xref} fields in \secref{bib:fld:spc} as well as \secref{bib:cav:ref} for further information concerning cross"=referencing. If \prm{variantspec} is specified, applies to a particular list variant (see \secref{bib:fld:var}).

\cmditem{ifnamexref}[variantspec]{name list}{true}{false}

Similar to \cmd{iffieldxref} but checks if a \prm{name list} is related to the cross"=referenced parent entry. See the description of the \bibfield{crossref} and \bibfield{xref} fields in \secref{bib:fld:spc} as well as \secref{bib:cav:ref} for further information concerning cross"=referencing. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).
...
\cmditem{ifcurrentform}{form}{true}{false}

Executes \prm{true} if the current variant form is \prm{form}, and \prm{false} otherwise. This command is robust. It is intended for use in list formatting directives and always executes \prm{false} when used in any other context. See \secref{bib:fld:var}.

\cmditem{ifcurrentlang}{lang}{true}{false}

Executes \prm{true} if the current variant language is \prm{lang}, and \prm{false} otherwise. This command is robust. It is intended for use in list formatting directives and always executes \prm{false} when used in any other context. See \secref{bib:fld:var}.
...
\cmditem{iffieldint}[variantspec]{field}{true}{false}

Similar to \cmd{ifinteger}, but uses the value of a \prm{field} rather than a literal string in the test. If the \prm{field} is undefined, it executes \prm{false}. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}). Note that this test doesn't work in an \cmd{ifthenelse} test due to internals conflicts but can be used with \cmd{ifboolexpr}.

\cmditem{iffieldnum}[variantspec]{field}{true}{false}

Similar to \cmd{ifnumeral}, but uses the value of a \prm{field} rather than a literal string in the test. If the \prm{field} is undefined, it executes \prm{false}. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).

\cmditem{iffieldnums}[variantspec]{field}{true}{false}

Similar to \cmd{ifnumerals}, but uses the value of a \prm{field} rather than a literal string in the test. If the \prm{field} is undefined, it executes \prm{false}. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).

\cmditem{iffieldpages}[variantspec]{field}{true}{false}

Similar to \cmd{ifpages}, but uses the value of a \prm{field} rather than a literal string in the test. If the \prm{field} is undefined, it executes \prm{false}. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).
...
\cmditem{iffieldbibstring}[variantspec]{field}{true}{false}

Similar to \cmd{ifbibstring}, but uses the value of a \prm{field} rather than a literal string in the test. If the \prm{field}  is undefined, it expands to \prm{false}. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).
...
\cmditem{ifbibfieldv}{bibfield}{true}{false}\BiberOnlyMark

Expands to \prm{true} or \prm{false} depending on whether the data source field \prm{bibfield} is a variant enabled field or not. See \secref{bib:fld:var}.

\cmditem{ifbibfieldvariants}{bibfield}{true}{false}\BiberOnlyMark

Expands to \prm{true} or \prm{false} depending on whether the data source field \prm{bibfield} has variants other than \opt{form} equal to the \opt{vform} option and \opt{lang} equal to the \opt{vlang} option (or the \opt{vtranslang} option when \kvopt{form}{translated}). See \secref{bib:fld:var}.

\cmditem{ifbibfieldform}{bibfield}{form}{true}{false}\BiberOnlyMark

Expands to \prm{true} or \prm{false} depending on whether the data source field \prm{bibfield} has a variant form \opt{form}. See \secref{bib:fld:var}.

\cmditem{ifbibfieldlang}{bibfield}{form}{lang}{true}{false}\BiberOnlyMark

Expands to \prm{true} or \prm{false} depending on whether the data source field \prm{bibfield} has a variant lang \opt{lang} for form \opt{form}. If \opt{form} is empty, the global \opt{vform} option value is used as the \opt{form}. See \secref{bib:fld:var}.
...
This command executes the macro \prm{name}, as defined with \cmd{newbibmacro}. If the macro takes any arguments, they are simply appended after \prm{name}. The regular form of this command sanitizes
\prm{name} while the starred form does not.
...
\cmditem{savelistelementformat}[entry type]{format}
\cmditem{restorelistelementformat}[entry type]{format}

These commands save and restore the formatting directive \prm{format}, as defined with \cmd{DeclareListElementFormat}. Both commands work within a local scope. They are mainly provided for use in localization files.
...
\cmditem{savenameelementformat}[entry type]{format}
\cmditem{restorenameelementformat}[entry type]{format}

These commands save and restore the formatting directive \prm{format}, as defined with \cmd{DeclareNameElementFormat}. Both commands work within a local scope. They are mainly provided for use in localization files.
...
\cmditem{iflistelementformatundef}[entry type]{name}{true}{false}
\cmditem{ifnameformatundef}[entry type]{name}{true}{false}
\cmditem{ifnameelementformatundef}[entry type]{name}{true}{false}
...
\cmditem{docsvfield}[variantspec]{field}

Similar to the \cmd{docsvlist} command from the \sty{etoolbox} package, except that it takes a field name as its argument. The value of this field is parsed as a comma"=separated list. If the \prm{field} is undefined, this command expands to an empty string. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).

\cmditem{forcsvfield}[variantspec]{handler}{field}

Similar to the \cmd{forcsvlist} command from the \sty{etoolbox} package, except that it takes a field name as its argument. The value of this field is parsed as a comma"=separated list. If the \prm{field} is undefined, this command expands to an empty string. If \prm{variantspec} is specified, applies to a particular field variant (see \secref{bib:fld:var}).
...
\cmditem{loopforms}{bibfield}{macro}\BiberOnlyMark

Loop over all variant forms of data source field \prm{bibfield}, doing \prm{macro} for each form.

\cmditem{loopformlangs}{bibfield}{form}{macro}\BiberOnlyMark

Loop over all variant languages of variant form \prm{form} of data source field \prm{bibfield}, doing \prm{macro} for each language.

\cmditem{loopvariants}{bibfield}{macro}\BiberOnlyMark

Loop over all variants of data source field \prm{bibfield}, doing \prm{macro} for each combination of \opt{form} and \opt{lang}. \prm{macro} should accept two parameters as it will be passed all \opt{form}, \opt{lang} combinations.

\cmditem{getlang}{bibfield}{form}\BiberOnlyMark

If data source field \prm{bibfield} has a single language variant for form \opt{form}, returns the language otherwise emits a warning.
...
The \cmd{setunit} command is similar to \cmd{newunit} except that it uses \prm{punctuation} instead of \cmd{newunitpunct}. The starred form differs from the regular version in that it checks if the last \cmd{printtext}, \cmd{printfield}, \cmd{printlist}, \cmd{printnames}, or \cmd{bibstring} command did actually print anything. If not, it does nothing.
...
A simplified but expandable version of \cmd{bibstring}. Note that this macro does not capitalize automatically, nor does it hook into the punctuation tracker. It is intended for special cases in which strings are nested or an expanded localization string is required in a test.
...
Switches from the current language to the main document language. This can be used in the \prm{wrapper} argument in the localization string commands above.
...
\csitem{currentform}\BiberOnlyMark

The name of the variant form currently being processes by a format. This information is only available locally in name formatting directives.

\csitem{currentlang}\BiberOnlyMark

The name of the variant language currently being processes by a format. This information is only available locally in name formatting directives.

\cntitem{variantcount}\BiberOnlyMark

This counter holds the number of variants which exist for a data source field. This information is only available locally in list formatting directives.

\cntitem{formcount}\BiberOnlyMark

This counter holds the number of variant forms which exist for a data source field. This information is only available locally in list formatting directives.

\cntitem{langcount}\BiberOnlyMark

This counter holds the number of variant languages which exist for a data source field. This information is only available locally in list formatting directives.

\cntitem{formlangcount}\BiberOnlyMark

This counter holds the number of variant languages which exist for the locally selected format form for a data source field. This information is only available locally in list formatting directives.
...
\item Added starred forms of \cmd{citeauthor} and \cmd{Citeauthor}\see{use:cit:txt}
...
\item Added starred form of \cmd{usebibmacro}\see{aut:aux:msc}
...
%\item Added starred form to \cmd{setunit}\see{aut:pct:new}
