...
\boolitem[false]{uniquetitle}\BiberOnlyMark
...
\end{ltxexample}
%
Here, the data for the citation of \bibfield{key2} will be drawn from refcontext \opt{labelprefix=A} and not \opt{labelprefix=A} (resulting in a label with prefix <A> and not <B>).
The unstarred commands do not override a local refcontext and so with:
...
\cmditem{ifcontentempty}{content}{true}{false}\BiberOnlyMark

Expands to \prm{true} if the \prm{contents} are empty and false otherwise. The \prm{contents} can consist of macros or strings and it will be expanded before the test.
...
\item Added \opt{uniquetitle} test\see{aut:aux:tst}\BiberOnlyMark
...
\item Added support for plural localisation strings with \bibfield{relatedtype}\see{aut:ctm:rel}\BiberOnlyMark
