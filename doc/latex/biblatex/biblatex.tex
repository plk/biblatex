...
Here is an example---suppose the \bibtex format bibliography contained the following:
...
\end{lstlisting}
%
Bear in mind that the bibliographies printed will still be sorted by their \opt{sort} option sorting specification (or the global sorting specification if this option is not set) and so you will almost certainly need to modify the sorting of these bibliographies to sort by the form you have chosen. You might want to switch these global settings but only in order to pick up fields which have the specified forms/langs. This is the purpose of the \cmd{vfallback*} commands:
...
\end{lstlisting}
%
Since \cmd{vfallbackon} is enabled by default, this means that even though the \bibfield{author} has no \opt{translated/english} variant (as it is by default \opt{original/french} due to the package options), it will be used anyway as a fallback. With \cmd{vfallbackoff}, the entry in the second bibliography would be missing the \bibfield{author} field since there is no \opt{translated/english} author field.

Setting \opt{autovlang=true} means that \biblatex will use whichever language is found for a field variant with form \opt{vform}. This assumes of course that your entry contains only one language variant with form \opt{vform} otherwise the behaviour will not be well defined. For example:
...
\end{lstlisting}
%
With fallback enabled and global variant settings \opt{vform} <original> and \opt{autovlang=true} the selected \bibfield{BOOKTITLE} and \bibfield{TITLE} fields for printing will be:
...
\end{lstlisting}
%
This is because the <original> language variants will be selected due to the \opt{autovlang} setting. If we reset the current \opt{vform} to <uniform> in the document with \cmd{vform}, then we would get:
\begin{lstlisting}[style=latex,escapechar=+]
TITLE     = Acta Barnabae
BOOKTITLE = Acta Apocryphorum Apostolorum
\end{lstlisting}
%
This is because the <uniform> variant of \bibfield{TITLE} would be selected, regardless of the language, due to the \opt{autovlang} setting but the \bibfield{BOOKTITLE} would fall back to the global <original> form and then the \opt{autovlang} setting would select whatever language variant of the <original> form exists.

\begin{ltxsyntax}
\cmditem{DeclareVariantFallbacks}{specification}\BiberOnlyMark

This defines variant fallbacks used by \biber when processing sorting, labels and inheritance. It has nothing to do with the general \biblatex fallbacks for \opt{vform} and \opt{vlang}. The \prm{specification} consists of one or more \cmd{vfallback} commands:
...
\boolitem[true]{autovlang}[\BiberOnly]

This option controls whether the language variant for a bibfield is looked for first in the field itself rather than using the global \opt{vlang} setting. See \secref{bib:fld:var}. This option can be set per"=entrytype and per"=entry.
...
\boolitem[false]{use$<$name$>$}

As per \opt{useauthor}, \opt{useeditor} and \opt{usetranslator}, all name lists defined in the data model have an option controlling their behaviour in sorting and labelling automatically defined. An internal toggle is automatically created for all name lists in the data model, called \opt{blx@useuse$<$name$>$}. Unlike \opt{useauthor}, \opt{useeditor} and \opt{usetranslator}, any other \opt{use$<$name$>$} options may not be set as a package option in \cmd{usepackage} but may be set with \cmd{ExecuteBibliographyOptions}. For example, after defining a new name list \bibfield{proofreader} field in the data model (see \secref{aut:ctm:dm}, you can add options to enable/disable its use in sorting/labelling by putting this in the preamble:

\begin{lstlisting}[style=latex]{}
\makeatletter
\DeclareBibliographyOption{useproofreader}[true]{%
  \settoggle{blx@useproofreader}{#1}}
\ExecuteBibliographyOptions{useproofreader=true}
\makeatother
\end{lstlisting}
