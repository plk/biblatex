...
\cmditem{DeprecateField}{oldfield}{newfield}

When an attempt is made to print \prm{oldfield}, \prm{newfield} is printed instead and a deprecation warning issued. This aids style authors who are changing field names in their style.
Complete compatibility is not assured with formatting etc. An effort is made to not break things using \prm{oldfield} but the warning means the source mentioning the old field name should be changed.

\cmditem{DeprecateList}{oldlist}{newlist}

As \cmd{DeprecateField} but for plain lists.

\cmditem{DeprecateName}{oldname}{newname}

As \cmd{DeprecateField} but for name lists.
...
\cmditem{DeclareSortTranslit}[entrytype]{specification}\BiberOnlyMark
...
\item Added \cmd{DeclareSortTranslit}\see{aut:ctm:srt}\BiberOnlyMark
