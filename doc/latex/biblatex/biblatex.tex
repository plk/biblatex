...
This option controls multilingual support. Multilignual support can be enabled for the bibliography lists, citations or both. When set to any \opt{auto} option, \sty{biblatex} will try to get the main document language from the \sty{babel} package (and fall back to English if \sty{babel} is not available). \opt{autobib} enables multilingual support in the bibliography only. This is the default behavior. \opt{autocite} enables multilingual support in citations. \opt{auto} enables both \opt{autobib} and \opt{autocite}. It is also possible to select the document language manually. In this case, the \opt{babel} option below will have no effect. Please refer to \tabref{bib:fld:tab1} for a list of supported languages and the corresponding identifiers.
...
Executes \prm{true} if there is data for entry with \prm{entrykey}, that is, it is in the bibliography. This is useful because an entry can be indirectly cited due to, for example, \opt{mincrossrefs} in which case \cmd{ifciteseen} and \cmd{ifentryseen} will return false even though the entry is in the bibliography.
...
\item Added \cmd{AtEveryMultiCite}, \cmd{AtNextMultiCite}, \cmd{UseEveryCiteHook}, \cmd{UseEveryCitekeyHook}, \cmd{UseEveryMultiCiteHook}, \cmd{UseNextCiteHook}, \cmd{UseNextCitekeyHook}, \cmd{UseNextMultiCiteHook}, \cmd{DeferNextCitekeyHook}\see{aut:fmt:hok}
\item Fixed \cmd{textcite} and related commands in the verbose styles\see{use:cit:cbx}
...
\item Added new citation delimiter \cmd{textcitedelim} for \cmd{textcite} and related commands to standard styles\see{aut:fmt:fmt}
\item Updated support for Russian (Oleg Domanov)
\end{release}
