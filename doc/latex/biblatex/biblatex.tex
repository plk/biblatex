...
\csitem{dateeraprint}{year}
Prints date era information when the global option \opt{dateera} is set to <secular> or <christian>. By default, prints the \opt{dateeradelim} delimiter and the appropriate localised era term (\secref{aut:lng:key:era}). If the \opt{dateeraauto} option is set, then the passed \prm{year} (usually via \cmd{thefield}) is tested to see if it is earlier than the \opt{dateeraauto} threshold and if so, then the CE/AD localisation will be output too. By default, when \opt{dateeraauto} is not used, no CE/AD localisation strings are output for CE/AD dates.

\csitem{enddateeraprint}{year}
As \cmd{dateeraprint} but used after end dates in a date range.

\csitem{dateeraprintpre}{year}
Prints date era information when the global option \opt{dateera} is set to <simple>. By default, prints \opt{bibdataeraprefix}.

\csitem{enddateeraprintpre}{year}
