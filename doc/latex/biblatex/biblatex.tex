...
\end{ltxsyntax}
%
Use \cmd{vfallbackon} to turn variant fallback on and \cmd{vfallbackoff} to turn it off. The default is \cmd{vfallbackon}. Variant fallback means that if a particular variant is not found for a field, a fallback will be looked for using the global default form and lang (specified via the package options). Such a fallback ignores any currently modified global values set by using \cmd{vform}, \cmd{vlang} or \cmd{vtranslang}. For example, perhaps you have an entry like this:
