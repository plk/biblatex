...
\lstset{basicstyle=\ttfamily,keepspaces=true,upquote=true}
...
\end{lstlisting}
%
The body is one or more \bibfield{entry} elements inside the top-level \bibfield{entries} element and everything is in the \bibfield{bltx} namespace. An entry has \bibfield{id} attribute corresponding to a \bibtex\ entry key and a \bibfield{entrytype} attribute corresponding to a \bibtex\ entrytype. For example, the \biblatexml\
...
\begin{lstlisting}[language=xml]
  <bltx:entry id="key1" entrytype="book">
    ...
    <bltx:publisher>...</bltx:publisher>
    <bltx:location>...</bltx:location>
  </bltx:entry>
\end{lstlisting}
%
